...
class LangChainAgentToolLoopTest extends Specification {
    ...
    def 'should enter the tool loop for a skills-only request and inject the available-skills catalog'() {
        ...
        def runner = new LangChainAgentRunner(modelFactory: Stub(ChatModelFactory) { createModel(*_) >> model })
    ...
    def 'should compose user text with the input JSON as a single user message'() {
        ...
        def factory = Stub(ChatModelFactory) { createModel(*_) >> model }
        def runner = new LangChainAgentRunner(modelFactory: factory)
     ...
     *
     * <p>A {@code fs:} tool never travels as a descriptor — it travels as a bare name on
     * {@code nativeToolNames}, so this runner has to rebuild the descriptor itself and dispatch it
     * back through the same callback. Two things break silently if it does not: an {@code fs:}-only
     * agent takes the single-shot path and never gets a tool loop at all, and the model is offered
     * nothing under a name it was told exists.
     */
    def 'should serve the runner-native fs tools from nativeToolNames alone'() {
        given: 'a model that calls `read` on the first turn, then answers'
        List<ChatRequest> capturedRequests = []
        int calls = 0
        ChatModel model = [
            chat: { ChatRequest req ->
        ...
        and: 'a dispatcher standing in for the bridge that serves the fs: tools in-JVM'
        List<List<String>> dispatched = []
        ToolDispatcher dispatch = { String name, String args ->
            dispatched << [name, args]
        ...
        and: 'a request whose ONLY tools are runner-native: no descriptor, no brokered half'
        def runner = new LangChainAgentRunner(modelFactory: Stub(ChatModelFactory) { createModel(*_) >> model })
        def req = new AgentRunnerRequest(
            model: 'openai/gpt-5-mini', instruction: 'i', prompt: 'go',
            maxIterations: 5, tools: [], outputSchema: null, inputJson: null,
            toolSpecs: null, dispatch: dispatch, requestTimeoutSeconds: 0, goal: null,
            nativeToolNames: ['read', 'grep'])

        when:
        def answer = runner.run(req)

        then: 'the tool loop ran, rather than the single-shot path an empty toolSpecs used to force'
        answer == 'done'
        calls == 2

        and: 'the selected native names were advertised to the model, under their bare wire names'
        capturedRequests[0].toolSpecifications()*.name() as Set == (['read','grep'] as Set)

        and: 'and dispatched back into the driver JVM through the same callback'
        dispatched == [['read', '{"path":"a.txt"}']]
    }
}
