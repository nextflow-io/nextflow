...
class WaveClient {
    ...
    WaveAssets resolveAssets(TaskRun task, String containerImage) {
        ...
        if( dockerFileContent && condaRecipe ) {
            throw new IllegalArgumentException("Process '$task.quickName' declares both a 'conda' directive and a module bundle dockerfile that conflicts each other")
        }
        Path condaFile = null
        if( condaRecipe ) {
            ...
            if( isCondaFile(condaRecipe) ) {
                ...
                dockerFileContent = condaFileToDockerFile(condaFile)
            }
            else {
        ...
        if( dockerFileContent && containerImage ) {
            throw new IllegalArgumentException("Process '$task.quickName' declares both a 'container' directive and a module bundle dockerfile that conflicts each other")
        }
    ...
    protected String condaFileToDockerFile(Path condaFile) {
        """\
        FROM mambaorg/micromamba:0.25.0 \\
        COPY --chown=\$MAMBA_USER:\$MAMBA_USER ${condaFile.getFileName()} /tmp/conda.yml \\
    ...
    protected String condaRecipeToDockerFile(String recipe) {
        ...
        FROM mambaorg/micromamba:0.25.0 \\
        RUN \\
           micromamba install -y -n base -c defaults \\
           $recipe \\
           && micromamba clean -a -y
        """.stripIndent()
    }
