...
class WaveContainerResolver implements ContainerResolver {
    ...
    static final private List<String> DOCKER_LIKE = ['docker','podman','sarus','apple-container','smolvm']
    ...
    ContainerInfo resolveImage(TaskRun task, String imageName) {
        ...
        if( engine in DOCKER_LIKE ) {
            ...
            final ret = defaultResolver.resolveImage(task, image.target, image.hashKey)
            // keep the wave request key to track the container status
            return new ContainerInfo(ret.source, ret.target, ret.hashKey, image.requestKey)
        }
        else
