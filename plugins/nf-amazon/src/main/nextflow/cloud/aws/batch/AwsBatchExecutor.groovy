...
class AwsBatchExecutor extends Executor implements ExtensionPoint, TaskArrayExecutor {
    ...
    static String s3Cmd(String workDir, AwsOptions opts) {
        ...
        final cmd = "trap \"[[ -n \\\$pid ]] && { echo Terminating process \\\$pid | tee -a ${TaskRun.CMD_LOG}; kill -TERM \\\$pid; }\" TERM; trap \"{ ret=\$?; $aws ${TaskRun.CMD_LOG} ${workDir}/${TaskRun.CMD_LOG}||true; exit \$ret; }\" EXIT; $aws ${workDir}/${TaskRun.CMD_RUN} - | bash > >(tee ${TaskRun.CMD_LOG}) 2>&1 & pid=\$!; wait \$pid"
        return cmd
    }
    ...
    static String s5Cmd(String workDir, AwsOptions opts) {
        ...
        final kms = opts.storageKmsKeyId ? " --sse-kms-key-id $opts.storageKmsKeyId" : ''
        final requesterPays = opts.requesterPays ? ' --request-payer requester' : ''
        final cmd = "trap \"[[ -n \\\$pid ]] && { echo Terminating process \\\$pid | tee -a ${TaskRun.CMD_LOG}; kill -TERM \\\$pid; }\" TERM; trap \"{ ret=\$?; $cli cp${sse}${kms}${requesterPays} ${TaskRun.CMD_LOG} ${workDir}/${TaskRun.CMD_LOG}||true; exit \$ret; }\" EXIT; $cli cat ${workDir}/${TaskRun.CMD_RUN} | bash > >(tee ${TaskRun.CMD_LOG}) 2>&1 & pid=\$!; wait \$pid"
        return cmd
    }
