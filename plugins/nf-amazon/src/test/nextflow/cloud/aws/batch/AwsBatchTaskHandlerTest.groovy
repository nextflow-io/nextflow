...
class AwsBatchTaskHandlerTest extends Specification {
    ...
    def 'should render submit command' () {
        ...
        then:
        result.join(' ') == 'bash -o pipefail -c trap "[[ -n \\$pid ]] && { echo Terminating process \\$pid | tee .command.log; kill -TERM \\$pid; }" TERM; trap "{ ret=$?; aws s3 cp --only-show-errors .command.log s3://work/.command.log||true; exit $ret; }" EXIT; aws s3 cp --only-show-errors s3://work/.command.run - | bash > >(tee .command.log) 2>&1 & pid=$!; wait $pid'
    ...
    def 'should render submit command with s5cmd' () {
        ...
        then:
        result.join(' ') == 'bash -o pipefail -c trap "[[ -n \\$pid ]] && { echo Terminating process \\$pid | tee .command.log; kill -TERM \\$pid; }" TERM; trap "{ ret=$?; s5cmd cp .command.log s3://work/.command.log||true; exit $ret; }" EXIT; s5cmd cat s3://work/.command.run | bash > >(tee .command.log) 2>&1 & pid=$!; wait $pid'
        ...
        then:
        result.join(' ') == 'bash -o pipefail -c trap "[[ -n \\$pid ]] && { echo Terminating process \\$pid | tee .command.log; kill -TERM \\$pid; }" TERM; trap "{ ret=$?; s5cmd --debug cp --sse aws:kms --sse-kms-key-id kms-key-123 .command.log s3://work/.command.log||true; exit $ret; }" EXIT; s5cmd --debug cat s3://work/.command.run | bash > >(tee .command.log) 2>&1 & pid=$!; wait $pid'
