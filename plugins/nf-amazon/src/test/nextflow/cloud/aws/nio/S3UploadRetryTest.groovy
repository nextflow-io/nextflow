...
class S3UploadRetryTest extends Specification {
        ...
        ExecutableHttpRequest prepareRequest(HttpExecuteRequest request) {
            return new ExecutableHttpRequest() {
                ...
                HttpExecuteResponse call() {
                    ...
                    bodies << (provider.present ? provider.get().newStream().bytes : new byte[0])
    ...
    def 'a single-part upload survives a transient 503 -- the body is re-read on the retry'() {
        ...
        then: 'the SDK retried, and both attempts carried the whole payload'
        transport.attempts == 2
        // not a byte-for-byte comparison of the two bodies: the aws-chunked framing embeds a
        // per-attempt chunk signature, so only the payload itself is expected to be identical
        transport.bodies.every { new String(it).contains(new String(payload)) }
