...
class AwsBatchExecutorTest extends Specification {
    ...
    def 'should get array launch command' (){
        ...
        false   | false       | false   | 's3://foo/work/dir' | 'bash -o pipefail -c \'trap "{ kill -TERM \\$pid; }" TERM; trap "{ ret=$?; aws s3 cp --only-show-errors .command.log s3://foo/work/dir/.command.log||true; exit $ret; }" EXIT; aws s3 cp --only-show-errors s3://foo/work/dir/.command.run - | bash > >(tee .command.log) 2>&1 & pid=$!; wait $pid\''
        false   | false       | true    | 's3://foo/work/dir' | 'bash -o pipefail -c \'trap "{ kill -TERM \\$pid; }" TERM; trap "{ ret=$?; s5cmd cp .command.log s3://foo/work/dir/.command.log||true; exit $ret; }" EXIT; s5cmd cat s3://foo/work/dir/.command.run | bash > >(tee .command.log) 2>&1 & pid=$!; wait $pid\''
        and:
