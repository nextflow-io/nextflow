...
//        path.getObjectName() == path.blobName()
    ...
    def 'isDirectory should return false when the attribute lookup fails'() {
        given: 'a relative path has no container, so the attribute lookup throws and is swallowed'
