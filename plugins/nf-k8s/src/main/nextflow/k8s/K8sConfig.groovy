...
class K8sConfig implements ConfigScope {
    ...
    @ConfigOption
    @Description("""
        Whether the pod should honour the entrypoint defined by the image (default: false).

        When `false`, the launcher is run via the pod `command` attribute, which overrides the entrypoint point defined by the image.

        When `true`, the launcher is run via the pod `args` attribute, without altering the container entrypoint (requires a bash shell as the image entrypoint).
    """)
    final boolean entrypointOverride
    ...
    K8sConfig(Map opts) {
        ...
        entrypointOverride = opts.entrypointOverride != null ? opts.entrypointOverride as boolean : System.getenv('NXF_CONTAINER_ENTRYPOINT_OVERRIDE')
