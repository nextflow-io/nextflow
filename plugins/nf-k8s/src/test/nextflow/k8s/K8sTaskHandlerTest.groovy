...
class K8sTaskHandlerTest extends Specification {
    ...
    def 'should return container mounts' () {
        ...
        1 * wrapper.getBinDirs() >> []
        1 * wrapper.getWorkDir() >> Paths.get('/work/dir')
        mounts == ['/base_path', '/work/dir']
    ...
    def 'should merge pod options' () {
        ...
        opts = handler.getPodOptions()
        then:
        1 * handler.taskPodOptions() >> new PodOptions([[env:'HELLO', value:'WORLD']])
        1 * k8sConfig.getPodOptions() >> new PodOptions()
    ...
    def 'should get task pod options' () {
        given:
        def taskConfig = Mock(TaskConfig)
        def task = Mock(TaskRun)
        task.getConfig() >> taskConfig
        ...
        when:
        def podOptions = handler.taskPodOptions()
        then:
        taskConfig.get('pod') >> null
        and:
        podOptions == new PodOptions(null)

        when:
        podOptions = handler.taskPodOptions()
        then:
        taskConfig.get('pod') >> [ [env: 'FOO', value: 'BAR'] ]
        and:
        podOptions == new PodOptions([ [env: 'FOO', value: 'BAR'] ])
