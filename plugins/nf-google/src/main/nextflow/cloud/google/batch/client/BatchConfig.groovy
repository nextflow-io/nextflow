...
class BatchConfig implements ConfigScope {
    ...
    @ConfigOption
    @Description("""
        The list of exit codes that will trigger Google Batch to automatically retry the job (default: `[50001]`).
    ...
    BatchConfig(Map opts) {
        ...
        maxSpotAttempts = opts.maxSpotAttempts as int ?: DEFAULT_MAX_SPOT_ATTEMPTS
    ...
    static BatchConfig create(Session session) {
        final googleOpts = GoogleOpts.create(session)
        final result = googleOpts.batch
        result.googleOpts = googleOpts
        result.credentials = googleOpts.credentials
        return result
    }
