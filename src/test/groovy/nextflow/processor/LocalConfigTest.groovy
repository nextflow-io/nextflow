 ...
 */
class LocalConfigTest extends Specification {


    def testShell() {

        when:
        def config = new LocalConfig().setContext(my_shell: 'hello')
        config.shell = value
        then:
    ...
    def testErrorStrategy() {
        ...
        when:
        def config = new LocalConfig(map)
    ...
    def testGetTime() {
        ...
        when:
        def config = new LocalConfig().setContext(ten: 10)
        config.time = value
