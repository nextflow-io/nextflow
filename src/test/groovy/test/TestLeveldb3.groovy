...
class MyCollector3 {
   ...
   String asString(byte[] bytes) {
    ...
    class MyComparator implements DBComparator{
        ...
        private Comparable get( byte[] key, int i ) {

            def result = (Comparable)cache.get(i)
            if( result != null ) {
                cacheHits++
            }
            else {
        ...
        int compare(byte[] key1, byte[] key2) {
            ...
            println "Compare $i1 <=> $i2"
            def v1 = get(key1, i1)
            def v2 = get(key2, i2)
            def ret = v1 <=> v2
            return ret ? ret : i1 <=> i2
        }
...
def collector = new MyCollector3()

println "Begin"
def now = System.currentTimeMillis()
def text = new File('/Users/pditommaso/Downloads/mini') // hs_ref_GRCh38_chr1.fa
