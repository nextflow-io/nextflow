...
cd ${TEMP_DIR}
echo "Pushing pipeline to ${S3_REPO}"
$NXF_CMD push s3-remote-test-repo -repo "${S3_REPO}" -r main
...
echo "Pushing modified pipeline to ${S3_REPO}"
$NXF_CMD push s3-remote-test-repo -m "Update greeting message"
