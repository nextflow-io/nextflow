...
No tools and no input data file — beyond the OpenAI key and the `pi` runner image
every agent task runs in. It is safe to run locally as a first test of the
`nf-agent-pi` plugin.
...
- A container engine and the `pi` runner image — every agent task is containerized
  (see [the examples README](../README.md#requirements)). No input data file needed.
