 ...
 *
 *  `module_run` — a single generic tool whose `module` enum lists every
 *  in-scope process (here just `word_stats`). The LLM calls it with
 *  `{"module":"word_stats","args":{"text":"..."}}` and receives the JSON
 *  output back.
 *
 ...
 * so the agent can read the stats directly from the module_run result.
 */
process word_stats {
    input:
        text: String
...
/*
 * The agent:
 *  1. Calls `module_run` with `word_stats` to compute text statistics.
...
agent analyst {
    ...
    instruction '''\
        You analyse text. Work step by step:
          1. Call module_run with {"module":"word_stats","args":{"text":"<the text>"}} to
