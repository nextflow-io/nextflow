...
reading the metric each time, until it converges on the optimum. No tool image,
no data fetch.

## Purpose / What it demonstrates

This is a *loop* in the strict sense — not tool composition (different tools
chained once, as in [`goal-directed`](../09_goal-directed)) but **iterated execution of one
...
the tunable knob is a *declared process input*. `nf:module_run` feeds declared
...
1. **The `score_threshold` process** reads a fixed, committed labelled dataset
   ...
   `exec:` process — no container of its own.
...
[the examples README](../README.md#requirements)). The tool needs no image and
there is no data fetch — `scores.txt` is committed alongside the example.
