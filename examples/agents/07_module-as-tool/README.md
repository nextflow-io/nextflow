...
1. **`include` + `nf:module_run` = a tool.** `include { SKESA } from 'nf-core/skesa'`
   brings the module into scope; `tools 'nf:module_run'` then advertises it to the
...
|---|---|
| `include` + `nf:module_run` | An nf-core module becomes a tool named `SKESA` |
