...
// `fs:` tools. The route through the tools is decided at run time, and
    ...
    instruction '''\
        You triage bacterial isolate assemblies. Work step by step:
          ...
          4. Otherwise the assembly PASSES QC: annotate the contigs using the PROKKA tool,
             write a brief summary JSON to the work directory using the `write` tool,
             then reply
             "PASS <sample>: N50=<n50>, contigs=<contigs>, annotation=<path to the annotation file>".
        '''.stripIndent()

    // `nf:module_run` discovers all included modules (skesa, assemblyscan, prokka)
    // and surfaces EACH as its OWN tool (SKESA, ASSEMBLYSCAN, PROKKA) with an
    // enforced input schema.
    // `fs:*` enables the sandboxed file tools — read, write, edit, ls, grep and find —
    // in the agent work dir.
