...
class LinPathTest extends Specification {
    ...
    def 'should get target path' () {
        given:
        def folder = Files.createTempDirectory("tmp")
        def lid = folder.resolve('.meta')
        def data = folder.resolve('work')
        ...
        def lidFs = new FileHelper().getOrCreateFileSystemFor('lid', [enabled: true, store: [location: folder.toString()]] ) as LinFileSystem

        lid.resolve('12345/output1').mkdirs()
        lid.resolve('12345/path/to/file2.txt').mkdirs()
        lid.resolve('12345/.data.json').text = '{"type":"TaskRun"}'
        lid.resolve('12345/output1/.data.json').text = '{"type":"DataOutput", "path": "' + outputFolder.toString() + '"}'
        lid.resolve('12345/path/to/file2.txt/.data.json').text = '{"type":"DataOutput", "path": "' + outputFile.toString() + '"}'
        ...
        lid.resolve('5678/').mkdirs()
        lid.resolve('5678/.data.json').text = wfResultsMetadata
    ...
    def 'should get subobjects as path' (){
        given:
        def lid = wdir.resolve('.meta')
        def lidFs = new FileHelper().getOrCreateFileSystemFor('lid', [enabled: true, store: [location: lid.parent.toString()]] ) as LinFileSystem
        ...
        Path p = LinPath.getMetadataAsTargetPath(wf, lidFs, "12345", ["workflow", "repository"] as String[])
        then:
        p instanceof LinMetadataPath
        p.text == '"repo"'

        when: 'outputs'
        def outputs = new WorkflowOutputs(OffsetDateTime.now(), "lid://12345", [ new Parameter("Collection", "samples", ["sample1", "sample2"])])
        lidFs.store.save("12345/outputs", outputs)
        Path p2 = LinPath.getMetadataAsTargetPath(wf, lidFs, "12345", ["outputs"] as String[])
        then:
        ...
        LinPath.getMetadataAsTargetPath(wf, lidFs, "12345", ["no-exist"] as String[])
        then:
        ...
        when: 'null object'
        LinPath.getMetadataAsTargetPath(null, lidFs, "12345", ["no-exist"] as String[])
        then:
