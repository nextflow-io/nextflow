...
public class ScriptAstBuilder {
    ...
    private ConstantExpression string(ParserRuleContext ctx) {
        var text = ctx.getText();
        checkEscapes(text, ctx, text.startsWith(SLASH_STR)).forEach(this::collectSyntaxError);
    ...
    private static final List<String> GROOVY_KEYWORDS = List.of(
        ...
        // token is already disabled in ScriptLexer.g4, so this only relaxes the
        // AST-builder identifier check.)
