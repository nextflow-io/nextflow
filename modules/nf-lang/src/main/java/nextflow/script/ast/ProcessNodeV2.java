...
public class ProcessNodeV2 extends ProcessNode {
    ...
    /**
     * Process inputs are represented as a single record parameter.
     *
     * @param inputs
     */
    private static Parameter[] dummyParams(Parameter[] inputs) {
        var cn = new ClassNode(Record.class);
        for( var input : inputs ) {
            var fn = new FieldNode(input.getName(), Modifier.PUBLIC, input.getType(), cn, null);
            fn.setDeclaringClass(cn);
            cn.addField(fn);
        }
        var param = new Parameter(cn, "");
