...
class MultiRevisionRepositoryStrategy extends AbstractRepositoryStrategy {
    ...
    protected void checkBareRepo(boolean recurseSubmodules) {
        assert bareRepo
        /*
        ...
        if( !hasBareRepo() ) {
            ...
            final mutex = new FileMutex(target: file, timeout: 60_000, waitMessage: wait, errorMessage: err)
            try {
                mutex.lock { createBareRepo(recurseSubmodules) }
            }
            finally {
                file.delete()
            }
        }
        final updateRevision = revision ?: getRemoteDefaultBranch()
    ...
    private void createBareRepo(boolean recurseSubmodules) {
        assert provider
    ...
    String download(String revision, Integer deep, boolean recurseSubmodules) {
        ...
        checkBareRepo(recurseSubmodules)

        // Try to get the commit Id for the revision and abort if not found.
        this.revision ?= getRemoteDefaultBranch()
        final commitId = revisionToCommitWithBareRepo(this.revision)
        if( !commitId ) {
            throw new AbortScmOperationException("No commit found for revision ${this.revision} in project ${project}")
        }
        ...
        final mutex = new FileMutex(target: file, timeout: 60_000, waitMessage: wait, errorMessage: err)
        try {
            mutex.lock { createSharedClone(this.revision, recurseSubmodules) }
        }
        finally {
            file.delete()
        }
    ...
    void tryCheckout(String revision) throws RefNotFoundException {
        ...
        final downloadRevision = revision ?: getRemoteDefaultBranch()
    ...
    Ref checkoutRemoteBranch(String revision, boolean recurseSubmodules) {
        download(revision, 1, recurseSubmodules)
        return findHeadRef()
    }
    ...
    String getRemoteDefaultBranch() {
        try {
            ...
            log.debug("Unable to get default branch in bare repo or local clone for $project - ${e.getMessage()}")
            return null
        }
    }
    ...
    private void dropRevision(String revision, boolean force) {
        ...
        throw new AbortScmOperationException("Revision $revision for project `${project}` contains uncommitted changes -- won't drop it")
    }
