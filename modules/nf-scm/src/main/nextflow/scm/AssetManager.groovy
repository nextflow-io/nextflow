...
import static nextflow.scm.ScmConst.*
...
class AssetManager {
    ...
    protected AssetManager(RepositoryProvider p){
        this.providerConfigs = ProviderConfig.createDefault()
        this.provider = p
    }
     ...
     */
    AssetManager( String pipelineName, Map config, HubOptions hubOpts = null ) {
        assert pipelineName
        // build the object
        build(pipelineName, config, hubOpts)
    }

    AssetManager( String pipelineName, Map config, String revision, HubOptions hubOpts = null ) {
        assert pipelineName
        // build the object
        build(pipelineName, config, hubOpts, revision)
    }

    protected RepositoryFactoryLoader getRepositoryFactory(){
        return DefaultRepositoryFactoryLoader.instance
    }
     ...
     * @param config A {@link Map} holding the configuration properties defined in the scm file
    ...
    AssetManager build( String pipelineName, Map config = null, HubOptions hubOpts = null, String revision = null ) {
        ...
        this.providerConfigs = repositoryFactory.createFromMap(config)
    ...
    private void updateStrategy(RepositoryStrategyType type = null) {
        ...
        if( type == RepositoryStrategyType.LEGACY )
            ...
            throw new AbortScmOperationException("Not supported strategy $type")
        if( revision )
            setRevision(revision)
    ...
    void setStrategyType(RepositoryStrategyType type) {
        ...
        if( type == RepositoryStrategyType.LEGACY && isUsingMultiRevisionStrategy() ) {
            log.warn "Switching to legacy SCM repository management"
            updateStrategy(RepositoryStrategyType.LEGACY)
        } else if( type == RepositoryStrategyType.MULTI_REVISION && SysEnv.get('NXF_SCM_LEGACY') ) {
            log.warn "Received a request to change to multi-revision SCM repository management and NXF_SCM_LEGACY is defined. Keeping legacy strategy"
            updateStrategy(RepositoryStrategyType.LEGACY)
        } else {
    ...
    void validateProjectDir() {
        ...
        if( hub != configProvider ) {
            throw new AbortScmOperationException("A project with name: `$localPath` has already been downloaded from a different provider: `$configProvider`")
        }
    ...
    String checkHubProvider( HubOptions hubOpts ) {
        ...
        if( !providerNames.contains(result)) {
            def matches = BasicBolts.closest(providerNames, result) ?: providerNames
            def message = "Unknown repository provider: `$result`'. Did you mean?\n" + matches.collect { "  $it"}.join('\n')
            throw new AbortScmOperationException(message)
        }
    ...
    String resolveName( String name ) {
        ...
        if( ['./','../', '/' ].any(it->name.startsWith(it)) )
            throw new AbortScmOperationException("Not a valid project name: $name")
        ...
        try {
            ...
            if( url.protocol == 'file' ) {
                ...
                if( config ) {
                    ...
                    result = StringUtils.stripStart(url.path, '/')
                }
            }
    ...
    RepositoryProvider createHubProvider(String providerName) {
        ...
        if( !config )
            throw new AbortScmOperationException("Unknown repository configuration provider: $providerName")

        return repositoryFactory.createRepositoryProvider(config, project)
    }

    @PackageScope
    void setLocalPath(File path) {
        if (!strategy)
            initStrategy(getRevision())
    ...
    String getDefaultBranch() {
        ...
        return strategy?.getRemoteDefaultBranch()
     ...
     */
    String download( String revision = null, Integer deep = null, boolean recurseSubmodules = false) {
        assert project
        if( !strategy ) {
            throw new IllegalStateException("Strategy not initialized")
        }
        return strategy.download(revision, deep, recurseSubmodules)
    }
     ...
     */
    void clone(File directory,  String revision = null, Integer deep=null, boolean recurseSubmodules = false) {
        ...
        if( !uri )
            throw new AbortScmOperationException("Cannot find the specified project: $project")

        clone.setURI(uri)
        clone.setDirectory(directory)
        clone.setDepth(1)
        clone.setCloneSubmodules(recurseSubmodules)
     ...
     */
    void checkout( String revision = null, boolean recurseSubmodules = false) {
        try {
    ...
    void tryCheckout(String revision = null) throws RefNotFoundException {
        ...
        strategy.tryCheckout(revision)
    }


    protected Ref checkoutRemoteBranch(String revision, boolean recurseSubmodules) {
        assert project
        if( !strategy ) {
            throw new IllegalStateException("Strategy not initialized")
        }
        return strategy.checkoutRemoteBranch(revision, recurseSubmodules)
    }
    /**
     *
     * @param recurseSubmodules Update with recurse submodules
     * @param gitmodules The `gitmodules` attribute in the manifest makes it possible to enable/disable modules updating
     */
    void updateModules(boolean recurseSubmodules, def gitmodules) {

        if( !localPath )
            return // nothing to do

        final marker = new File(localPath, '.gitmodules')
        if( !marker.exists() || BasicFileHelper.empty(marker) )
            return

        final modules = gitmodules
        if( modules == false )
            return
    ...
    void drop(String revision = null, boolean force = false) {
        if( isNotInitialized() )
            throw new AbortScmOperationException("No match found for: ${getProjectWithRevision()}")
