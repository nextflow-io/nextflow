...
class OarExecutor extends AbstractGridExecutor {
    ...
    static private Map DECODE_STATUS = [
            'toLaunch': QueueStatus.RUNNING,			// (the OAR scheduler has attributed some nodes to the job which will be launched)
            ...
            'toAckReservation': QueueStatus.PENDING,	// (the OAR scheduler must say “YES” or “NO” to the waiting oarsub command which requested a reservation)
            ...
            'Suspended': QueueStatus.HOLD,				// (the job was in a Running state and there was a request (oarhold with “-r” option) to suspend this job. In this state other jobs can be scheduled on the same resources (these resources have “suspended_jobs” set to “YES”).)
