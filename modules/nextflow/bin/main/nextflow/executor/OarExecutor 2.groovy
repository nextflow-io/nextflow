...
class OarExecutor extends AbstractGridExecutor {
     ...
     * @param result the {@link List} instance to which to add the job directives
     ...
     *
     * @param task the {@link TaskRun} instance to submit to the cluster for execution
    ...
    static private Map DECODE_STATUS = [
            'toLaunch': QueueStatus.RUNNING,			// (the OAR scheduler has attributed some nodes to the job to be launched)
            ...
            'Waiting': QueueStatus.PENDING,				// (the job is waiting for the OAR scheduler decision)
            'toAckReservation': QueueStatus.PENDING,	// (the OAR scheduler must say “YES” or “NO” to the waiting oarsub command which has requested a reservation.)
            ...
            'Suspended': QueueStatus.HOLD,				// (the job was in Running state and there was a request (oarhold with “-r” option) to suspend this job. In this state, other jobs can be scheduled on the same resources (these resources have “suspended_jobs” set to “YES”))
            'Error': QueueStatus.ERROR,					// (a problem has occurred)
            'toError': QueueStatus.ERROR,				// (the job is entering an error state.)
