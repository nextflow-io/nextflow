...
/**
 * Builder to create the Bash script used to
...
class BashWrapperBuilder {
    ...
    static {
         ...
         * - 2: trace the execution of the user script, adding the `set -x` flag
    ...
    /**
     * @return the bash script fragment to change to the 'scratch' directory, if specified in the task configuration
     */
    protected String getScratchDirectoryCommand() {
         ...
         * try to use the 'TMP' variable, if it does not exist, fallback to a tmp folder
         */
        if( scratchStr == 'true' ) {
    ...
    protected Map<String,String> makeBinding() {
        ...
        // set true when running with a container engine
        runWithContainer = containerEnabled && !containerNative

        // whenever it has to change the scratch directory
        ...
        // patch root ownership problems on files created with docker
    ...
    /**
     * Build up the BASH wrapper script file that will launch the user-provided script
    ...
    private Path write0(Path path, String data) {
        try {
            ...
            // via the nextflow re-try mechanism
    ...
    String getCleanupCmd(String scratch) {
        ...
        // -- remove the container in this way, because 'docker run --rm'  fails in some cases -- see https://groups.google.com/d/msg/docker-user/0Ayim0wv2Ls/-mZ-ymGwg8EJ
    ...
    ContainerBuilder createContainerBuilder(String changeDir) {
        ...
        // add the user owner variable in order to patch root-owned file problems
        if( fixOwnership() )
        ...
        // when secrets are not managed by the execution platform natively
        // the secret names are added to the container env var whitelist
        ...
        // the task is executed in a temporary scratch directory (i.e. changeDir != null)
