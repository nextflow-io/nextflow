...
class Session implements ISession {
    ...
    /**
     * Keeps a list of all processors created
     */
    ...
    /**
     * Enables / disables tasks result caching
     */
    boolean cacheable

    /**
     * True if launched in resume mode
     */
    boolean resumeMode

    /**
     * The folder where task temporary files are stored
     */
    Path workDir
    ...
    /**
     * The pipeline script name (without the parent path)
     */
    String scriptName
    ...
    /**
     * List files that are concurrent on the session configuration
     */
    ...
    /*
     * Disables the upload of the project 'bin' directory when using a cloud executor
     */
    ...
    /**
     * Creates a new session, given the configuration specified
     *
     * @param config
     */
    Session(Map obj) {
    ...
    /**
     * @return the session's max number of threads allowed
     */
    ...
    Session init( ScriptFile scriptFile, List<String> args=null ) {
        ...
        if(!workDir.mkdirs()) throw new AbortOperationException("Cannot create work-dir: $workDir -- esure you have write permissions, or specify a different directory using the `-w` command line option")
        ...
        if( scriptFile ) {
            ...
            // sets the script name attribute
            this.setScriptName(scriptFile.main.name)
        }

        // sets the byte-code target directory
     ...
     *
     * @param runOpts the {@code CmdRun} object that contains the run command options
    ...
    List<TraceObserver> createObservers() {
        ...
        // stats is created, first because others may depend on it
     ...
     * - on the third, force termination with System#exit
     */
    ...
    /**
     * Dump the current dataflow network that lists
    ...
    Session start() {
        ...
        // register shutdown cleanup hooks
    ...
    /**
     * The folder where script binary files are located, by default 'bin'
    ...
    final protected void joinAllOperators() {
        ...
        while( allOperators.size() ) {
            if( attempts++>0 )
                log.debug "Unexpected behavior -- attempt number $attempts to join pending operators"
    ...
    /**
     * Halt the pipeline execution, choosing to exit immediately, or complete the current
     ...
     *
     * @param cause a {@link Throwable} instance representing the task that caused the pipeline execution to abort
     */
    ...
    /**
     * Check preconditions before running the main script
     */
    ...
    @PackageScope void checkConfig() {
        ...
        if( enabled ) {
            ...
            log.debug "Config process name validation disabled as requested"
        }
    }
    ...
    @PackageScope void checkVersion() {
        ...
        // when the version string is prefied with `!`,
        // an exception is thrown if the version does not match
    ...
    @PackageScope void showVersionWarning(String ver) {
        log.warn "Nextflow version $Const.APP_VER does not match workflow required version: $ver -- execution will continue, but may produce errors"
    }
     ...
     * @return {@code true} if the name specified belongs to the list of process names, otherwise {@code false}
     */
    protected boolean checkValidProcessName(Collection<String> processNames, String selector, List<String> errorMessage)  {
        ...
        def message = "No process found matching config selector: $selector"
        if( suggestion )
    ...
    void notifyTaskSubmit( TaskHandler handler ) {
        ...
        // -- saves a record in the cache index
    ...
    void notifyTaskComplete( TaskHandler handler ) {
        // saves the completed task in the cache DB
        final trace = handler.safeTraceRecord()
        cache.putTaskAsync(handler, trace)

        // notifies the event to the observers
        for( int i=0; i<observers.size(); i++ ) {
            final observer = observers.get(i)
            try {
    ...
    void notifyTaskCached( TaskHandler handler ) {
        final trace = handler.getTraceRecord()
        // saves a record in the cache index only the when the trace record is available,
        // otherwise the event was triggered by a `stored dir` driven task
        if( trace ) {
    ...
    /**
     * Delete the workflow work directory from task temporary files
     */
    void cleanup() {
    ...
    def fetchContainers() {
        ...
        if( config.process instanceof Map<String,?> ) {
            ...
            /*
             * look for a `container` definition at process level
             */
    ...
    /**
     * Resolve dynamically-defined attributes to their actual value
     *
     ...
     * @param defValue the default value, if the setting is not defined in the configuration file
     * @return the number of tasks to handle in parallel
     */
    @Memoized
    ...
    /**
     *  Determines how long the executors waits before returning an error status when a process is
     *  terminated, but the exit file does not exist, or it is empty. This setting is used only by grid executors
     *
     * @param execName the executor name
     * @param defValue the default value, if the setting is not defined in the configuration file
     * @return a {@code Duration} object. Default '90 seconds'
     */
    @Memoized
