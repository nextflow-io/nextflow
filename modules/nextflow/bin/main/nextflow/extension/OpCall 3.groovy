...
/**
 * Represents a nextflow operation invocation
 *
...
class OpCall implements Callable {
    ...
    OpCall setSource(ChannelOut left) {
        ...
        if( args.size() )
            throw new ScriptRuntimeException("Multi-channel output cannot be applied to operator ${methodName} -- an argument is already provided")
    ...
    private <T> T read0(source){
        ...
        // Keep track of this relationship, to retrieve the
    ...
    protected Object invoke() {
        if( methodName==SET_OP_hack ) {
            // the problem is that `set` is not a real operator,
    ...
    protected void checkDeprecation(Method method) {
        if( method.getAnnotation(Deprecated) ) {
            def messg = "Operator `$methodName` is deprecated -- will be removed in a future release"
            if( NF.isDsl2() )
            ...
            def messg = annot.message() ?: "Operator `$methodName` is deprecated -- unavailable in DSL2 syntax".toString()
            throw new DeprecationException(messg)
        }
    }
    ...
    /**
     * Given an array of arguments, copy the first n-1 to a list of {@link groovyx.gpars.dataflow.DataflowWriteChannel}s
     */
    @CompileStatic
