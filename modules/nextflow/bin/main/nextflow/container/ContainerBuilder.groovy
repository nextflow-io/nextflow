...
abstract class ContainerBuilder<V extends ContainerBuilder> {
    ...
    /**
     * Get the env command line option for the given environment definition
     *
     * @param env
     * @param result
     * @return
    ...
    protected CharSequence makeVolumes(List<Path> mountPaths, StringBuilder result = new StringBuilder() ) {
        ...
        // find the longest common paths to mount
        ...
        // -- append the current path by default -- this is needed when `scratch` is true
        if( mountWorkDir ) {
