...
class SingularityBuilderTest extends Specification {
    ...
    def 'should get the run command line' () {
        ...
        new SingularityBuilder('ubuntu')
            .params(oci: true)
            .build()
            .runCommand == 'set +u; env - PATH="$PATH" ${TMP:+SINGULARITYENV_TMP="$TMP"} ${TMPDIR:+SINGULARITYENV_TMPDIR="$TMPDIR"} ${XDG_RUNTIME_DIR:+XDG_RUNTIME_DIR="$XDG_RUNTIME_DIR"} ${DBUS_SESSION_BUS_ADDRESS:+DBUS_SESSION_BUS_ADDRESS="$DBUS_SESSION_BUS_ADDRESS"} singularity exec --no-home --pid --oci -B "$PWD" ubuntu'
