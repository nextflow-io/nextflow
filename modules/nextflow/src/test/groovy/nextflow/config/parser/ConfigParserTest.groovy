...
class ConfigParserTest extends Specification {
    ...
    def 'should load selected profile configuration' () {
        ...
        when:
        def config2 = new ConfigParserImpl()
                        .setProfiles(['fast'])
                        .parse(configText)
        then:
    ...
    def 'should return the set of parsed profiles' () {
        ...
        when:
        slurper = new ConfigParserImpl().setProfiles(['omega'])
        slurper.parse(text)
        then:
        slurper.getProfiles() == ['alpha','beta'] as Set
    }
    ...
    def 'should disable includeConfig parsing' () {
        ...
        when:
        new ConfigParserImpl().parse(text)
        then:
    ...
    def 'should parse file named as a top config scope' () {
        ...
        when:
        new ConfigParserImpl().parse(configFile)
        then:
        noExceptionThrown()
