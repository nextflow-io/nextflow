...
class ToolOutputReaderTest extends Specification {
    ...
    def 'should preserve the scheme for a remote path handle'() {
        given:
        final provider = Mock(FileSystemProvider)
        final fs = Mock(FileSystem)
        final file = Mock(Path)
        file.getFileName() >> Path.of('contigs.fa')
        file.getFileSystem() >> fs
        fs.provider() >> provider
        provider.getScheme() >> 'mock'
        file.toAbsolutePath() >> file
        file.toUri() >> URI.create('mock://bucket/work/contigs.fa')

        when:
        final result = ToolOutputReader.readOrHandle(file, 1024 * 1024)

        then:
        result == 'mock://bucket/work/contigs.fa'
    }
