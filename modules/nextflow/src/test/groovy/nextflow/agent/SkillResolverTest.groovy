...
class SkillResolverTest extends Specification {
    ...
    def 'should fetch a remote skill via a file:// clone and reuse the cache'() {
        ...
        and: 'the clone is cached under the reserved .remote segment, never beside the local skills'
        Files.isDirectory(base.resolve('skills/.remote/myrepo'))
        !Files.exists(base.resolve('skills/myrepo'))
    ...
    // A module ships its hand-authored skills in the very same `<moduleDir>/skills` directory that
    // the remote clone cache lives under, so the clone cache MUST NOT be able to be satisfied by a
    // same-named LOCAL skill dir -- otherwise the declared remote skill is silently never fetched
    // and the model is instructed with the wrong content (with no warning, and folded into the
    // resume fingerprint as if it were the remote skill).
    def 'should not serve a remote skill from a same-named local skill directory'() {
        given: 'a hand-authored local skill whose name equals the remote repo name'
        def skillsRoot = Files.createDirectories(tmp.resolve('mod/skills'))
        Files.createDirectories(skillsRoot.resolve('style'))
        Files.writeString(skillsRoot.resolve('style/SKILL.md'), "---\nname: style\ndescription: local\n---\nLOCAL-CONTENT-NOT-FROM-GITHUB")

        when: 'the agent declares the skill as a remote ref (an unreachable LOCAL url, so the test is hermetic)'
        SkillResolver.loadRemoteUrl(skillsRoot, 'file:///does/not/exist.git', 'style', null)

        then: 'the fetch is attempted and fails -- the local dir is NOT mistaken for the clone cache'
        def e = thrown(ScriptRuntimeException)
        e.message.contains('Unable to fetch remote skill')
        and: 'the local skill is untouched and still resolvable by name'
        SkillResolver.loadLocal(skillsRoot, 'style')[0].content == 'LOCAL-CONTENT-NOT-FROM-GITHUB'
    }

    // The reserved `.remote` segment must not become a traversal foothold: `repoName` comes from the
    // ref regex, which accepts `..`, so `<skillsRoot>/.remote/..` normalizes back to skillsRoot --
    // still CONTAINED in skillsRoot, so a containment-only guard passes it. The cache would then be
    // seen as already populated and every hand-authored module skill would be served (and
    // fingerprinted) as the remote skill's content, silently.
    def 'should reject a traversing remote skill repo name instead of serving the module skills'() {
        given: 'a module skills dir holding a hand-authored skill'
        def skillsRoot = Files.createDirectories(tmp.resolve('mod/skills'))
        Files.createDirectories(skillsRoot.resolve('style'))
        Files.writeString(skillsRoot.resolve('style/SKILL.md'), "---\nname: style\ndescription: local\n---\nLOCAL-CONTENT")

        when: 'the repo name climbs out of the reserved cache dir'
        SkillResolver.loadRemoteUrl(skillsRoot, 'file:///does/not/exist.git', '..', null)

        then: 'it is rejected outright -- NOT resolved to skillsRoot and served as a warm cache'
        def e = thrown(ScriptRuntimeException)
        e.message.contains('Invalid skills cache path')
        and: 'the module skill was never scanned as remote content'
        SkillResolver.loadLocal(skillsRoot, 'style')[0].content == 'LOCAL-CONTENT'
    }
    ...
    def 'should sanitize a slash-bearing rev into a flat cache dir name (no path traversal/nesting)'() {
        ...
        Files.isDirectory(skillsRoot.resolve('.remote/repo@feature_x'))
        and: 'no nested repo@feature/x path was created'
        !Files.exists(skillsRoot.resolve('.remote/repo@feature'))
    }
}
