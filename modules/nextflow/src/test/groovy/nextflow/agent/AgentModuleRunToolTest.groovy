...
/**
 * Integration test for the {@code nf:module_run} family: an agent declares
 * {@code tools 'nf:module_run'} and every in-scope / included process is exposed to the LLM
...
class AgentModuleRunToolTest extends Dsl2Spec {
    ...
    def 'should expose an included classic module (meta.yml-driven) as its own tool with an ENFORCED flattened schema, run it, and whitelist its output dir'() {
        ...
        AgentRunnerProvider.testRunner = { AgentRunnerRequest req ->
            captured = req
            // §5 partition: `toolSpecs` is the BROKERED half only -- the per-module tool FOO,
            // never an aggregate 'module_run'. The six `fs:*` leaves are runner-native and travel
            // beside it as bare names, never as descriptors (and never as one 'filesystem' tool).
            assert req.toolSpecs.size() == 1
            assert req.toolSpecs*.name == ['FOO']
            assert req.nativeToolNames == ['read','write','edit','ls','grep','find']
            ...
            // (d) after the run, the dispatch context's readable paths include the module output
            //     FILE itself -- so a subsequent filesystem read of it is allowed, while its
            //     siblings stay outside the sandbox
            final ctx = getDispatchContext()
            assert ctx != null
            final outPath = Path.of(outfile)
            assert ctx.readablePaths.contains(outPath)
            assert !ctx.readablePaths.contains(outPath.getParent())
    ...
    def 'nf:module_run and fs:* together produce a per-module tool plus the six fs leaves'() {
        given:
        AgentRunnerProvider.testRunner = { AgentRunnerRequest req ->
            // the brokered per-module tool (greet) in `toolSpecs`, the six `fs:*` leaves beside
            // it in `nativeToolNames` -- the two halves of the §5 partition, never mixed
            assert req.toolSpecs.size() == 1
            assert req.toolSpecs*.name == ['greet']
            assert req.toolSpecs*.name.every { it != 'module_run' }
            assert req.nativeToolNames == ['read','write','edit','ls','grep','find']
            // the partition holds structurally, not just by inspection
            req.checkToolPartition()
            assert req.brokeredToolNames() == (['greet'] as Set)
            ...
            // an in-JVM runner still serves the fs: leaves itself, through the same dispatcher
            final listed = new JsonSlurper().parseText(req.dispatch.call('ls', '{}')) as Map
            assert listed.entries != null
            return result
        } as AgentRunner
