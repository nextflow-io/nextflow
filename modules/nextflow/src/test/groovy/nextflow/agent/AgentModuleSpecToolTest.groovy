 ...
 *
 * The agent {@code include}s the module and declares {@code tools 'nf:module_run:echo_tool'};
 * the bridge finds the sibling
...
class AgentModuleSpecToolTest extends Dsl2Spec {
    ...
    def 'should read ALL outputs of a multi-output tool (not block on the 2nd)'() {
        ...
        dir.resolve('main.nf').text = '''
            include { two_out } from './mod.nf'

            agent a {
                model 'm'
                instruction 'i'
                tools 'nf:module_run:two_out'
