...
class SandboxGuardTest extends Specification {
    ...
    def 'should handle null elements in readablePaths without throwing'() {
        given:
        def work = Files.createDirectories(tmp.resolve('work'))
        def mod = Files.createDirectories(tmp.resolve('moduleout'))
        def f = mod.resolve('result.fa')
        def readablePathsWithNull = Arrays.asList(null, mod)
        expect:
        SandboxGuard.isAllowed(f, work, readablePathsWithNull, false)
        !SandboxGuard.isAllowed(f, work, readablePathsWithNull, true)
    }
}
