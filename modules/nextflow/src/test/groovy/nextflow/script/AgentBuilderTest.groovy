...
class AgentBuilderTest extends Specification {
    ...
    def 'should accumulate the repeatable label directive'() {
        given:
        def builder = new AgentBuilder(null, 'a')
        def single = new AgentBuilder(null, 'b')
        ...
        and:
        single.label('a')
        single._input_('q', String); single._output_('a', String)
        def a2 = single.withPrompt(new PromptDef({ 'x' }, 'x')).build()

        then: 'repeated invocations accumulate instead of overwriting'
        a1.labels == ['a', 'b']
        and: 'a single label is still a list'
        a2.labels == ['a']
    }
