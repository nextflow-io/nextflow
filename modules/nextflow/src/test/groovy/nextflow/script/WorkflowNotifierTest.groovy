...
class WorkflowNotifierTest extends Specification {
    ...
    def 'should create notification mail' () {
        ...
        notifier.setProperty('workflow',  workflow)
    ...
    def 'should send notification email' () {
        ...
        when:
        notifier.setProperty('workflow',  workflow)
        notifier.setProperty('config', [notification: CONFIG_NOTIFIER, mail: CONFIG_MAIL])
        ...
        '''
        notifier.setProperty('config', [notification: [enabled: false, to:'you@dot.com']])
        notifier.sendNotification()
        then:
        ...
        '''
        notifier.setProperty('config', [notification: [to:'you@dot.com']])
        notifier.sendNotification()
        then:
        0 * notifier.createMail(_) >> null
        0 * notifier.createMailer(_) >> MAILER
