...
class AgentScriptLoadingTest extends Dsl2Spec {
    ...
    def 'should support helper statements before the prompt text'() {
        given:
        def session = new Session()
        def parser = new ScriptLoaderV2(session)
        def file = Files.createTempDirectory('test').resolve('main.nf')
        ...
        parser.parse(file)
        parser.runScript()
        def agent = ScriptMeta.get(parser.script).getDefinitions()
            .find { it instanceof AgentDef && it.name == 'qa' } as AgentDef
        def prompt = (Closure) agent.prompt.closure.clone()
        prompt.setDelegate([question: 'fastq'])
        prompt.setResolveStrategy(Closure.DELEGATE_FIRST)

        then: 'the prompt is the block\'s last expression, with the helpers in scope'
        prompt.call().toString().trim() == 'Write a haiku about FASTQ'

        and: 'the source text spans the whole block, so the helpers enter the cache key'
        agent.prompt.source.contains('def topic')
    ...
    def 'should load a script with namespaced tool refs (nf:module_run + fs:*) and expose them via getTools()'() {
        given:
        def session = new Session()
        def parser = new ScriptLoaderV2(session)
        def file = Files.createTempDirectory('test').resolve('main.nf')
        ...
        agent != null
        agent.tools.contains('nf:module_run')
        agent.tools.contains('fs:*')
        agent.tools.size() == 2
    ...
    def 'should load a script with a skills directive and expose it via getSkills()'() {
        given:
        def session = new Session()
        def parser = new ScriptLoaderV2(session)
        def file = Files.createTempDirectory('test').resolve('main.nf')
        file.text = '''
            ...
            agent helper {
                ...
                skills 'greet', 'github.com/org/repo'
        ...
        then:
        def definitions = ScriptMeta.get(parser.script).getDefinitions()
        def agent = definitions.find { it instanceof AgentDef && it.name == 'helper' } as AgentDef
        agent != null
        agent.skills == ['greet', 'github.com/org/repo']
