 ...
 */
class MapOutParamTest extends Dsl2Spec {
    ...
    def 'should define output maps'() {
        ...
        def text = '''
            process hola {
              output:
                map( x: val(x) )
                map( y: val(y), stdout: stdout, fa: file('*.fa')  )
                map( stdout: stdout, z: val(z) )
                map( foo: env(FOO), bar: env(BAR) )
        ...
        when:
        MapOutParam out0 = process.config.getOutputs().get(0)
        MapOutParam out1 = process.config.getOutputs().get(1)
        MapOutParam out2 = process.config.getOutputs().get(2)
        MapOutParam out3 = process.config.getOutputs().get(3)
