 ...
 */
class RecordInParamTest extends Dsl2Spec {
    ...
    def 'should create input maps'() {
        ...
        def text = '''
            ...
            process hola {
              input:
              record( p: val(p) )
              record( p: val(p), q: val(q) )
              record( v: val(v), fa: path('file_name.fa') )
              record( v: val(v), txt: path('file_name.txt'), stdin: '-' )
              record( p: val(p), z: path(z, stageAs: 'file*') )
              /foo/
            }
            workflow {
              hola(x, x, 'str', 'ciao', x)
        ...
        RecordInParam in1 = process.config.getInputs().get(0)
        RecordInParam in2 = process.config.getInputs().get(1)
        RecordInParam in3 = process.config.getInputs().get(2)
        RecordInParam in4 = process.config.getInputs().get(3)
        RecordInParam in5 = process.config.getInputs().get(4)
