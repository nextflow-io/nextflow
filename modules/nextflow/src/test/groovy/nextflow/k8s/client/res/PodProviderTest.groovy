 ...
 */
class PodProviderTest extends Specification {

    def newProvider() {
        Spy(PodProvider) {
            config >> [namespace: 'default']
            makeRequest(_, _) >> null
            trace(_, _, _) >> null
        }
    }
    ...
    def 'should delete a pod' () {
        ...
        then:
        1 * client.makeRequest('DELETE','/api/v1/namespaces/default/pods/foo') >> RESP
        result.field == 'OK'
        ...
        1 * client.makeRequest('DELETE','/api/v1/namespaces/bar/pods/foo') >> RESP
        result.field == 'OK'
    ...
    def 'should list pods' () {
        ...
        when:
        result = client.list(true)
        then:
        1 * client.makeRequest('GET','/api/v1/pods') >> RESP
        result.response == 'hello'

        when:
        result = client.list()
        then:
        1 * client.makeRequest('GET','/api/v1/namespaces/default/pods') >> RESP
        result.response == 'hello'
        ...
        1 * client.makeRequest('GET','/api/v1/namespaces/foo/pods') >> RESP
        result.response == 'hello'
    }

    def 'should fetch pod log' () {
        ...
        when:
        result = client.fetchLog('pod-123')
        then:
        1 * client.makeRequest('GET','/api/v1/namespaces/default/pods/pod-123/log') >> RESP
        result == STREAM

        when:
        result = client.fetchLog('pod-123', follow: true)
        then:
        1 * client.makeRequest('GET','/api/v1/namespaces/default/pods/pod-123/log?follow=true') >> RESP
        result == STREAM

        when:
        result = client.fetchLog('pod-123', follow: true, foo: 1, bar: 'x')
        then:
        1 * client.makeRequest('GET','/api/v1/namespaces/default/pods/pod-123/log?follow=true&foo=1&bar=x') >> RESP
        result == STREAM
    ...
    def 'should get a pod state' () {
        ...
        when:
        def result = client.getState(POD_NAME)
        then:
        1 * client.getPodStatus0(POD_NAME) >> new K8sResponseJson(JSON)
    ...
    def 'should get a pod its node' () {
        ...
        when:
        def result = client.getHostname(POD_NAME)
        then:
        1 * client.getPodStatus0(POD_NAME) >> new K8sResponseJson(JSON)
    ...
    def 'should return a process execution on pod not found' () {
        ...
        when:
        client.getState(POD_NAME)
        then:
        1 * client.getPodStatus0(POD_NAME) >> { throw new K8sResponseException("Request GET /api/v1/namespaces/xyz/pods/nf-xyz/status returned an error code=404", new ByteArrayInputStream(JSON.bytes)) }
    ...
    def 'should fail to get pod state' () {
        ...
        client.getState(POD_NAME)
        then:
        1 * client.getPodStatus0(POD_NAME) >> new K8sResponseJson([:])
        ...
        client.getState(POD_NAME)
        then:
        1 * client.getPodStatus0(POD_NAME) >> new K8sResponseJson([status:[containerStatuses: []]])
        e = thrown(K8sResponseException)
        e.message.startsWith('K8s undetermined status conditions for pod')
        ...
        client.getState(POD_NAME)
        then:
        1 * client.getPodStatus0(POD_NAME) >> new K8sResponseJson([status:[containerStatuses: [ [name: 'foo'] ]]])
        e = thrown(K8sResponseException)
        ...
        client.getState(POD_NAME)
        then:
        1 * client.getPodStatus0(POD_NAME) >> new K8sResponseJson([status:[containerStatuses: [ [name: POD_NAME] ]]])
        e = thrown(K8sResponseException)
        ...
        def result = client.getState(POD_NAME)
        then:
        1 * client.getPodStatus0(POD_NAME) >> new K8sResponseJson([status:[containerStatuses: [ [name: POD_NAME, state: STATE] ]]])
        result == STATE
    }
