...
class WebLogObserverTest extends Specification {
    ...
    def 'should create a json message' () {
        ...
        observer.setProperty('endpoint', 'http://foo.com')
        observer.setProperty('httpClient', CLIENT)
        observer.setProperty('runName', 'foo')
        observer.setProperty('runId', 'xyz')
        observer.setProperty('generator', new JsonGenerator.Options().build())
