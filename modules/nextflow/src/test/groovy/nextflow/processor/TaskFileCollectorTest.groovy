...
class TaskFileCollectorTest extends Specification {
    ...
    def 'should filter staged inputs'() {
        ...
        def task = Spy(new TaskRun(
                ...
                workDir: workDir ))
        def collector = new TaskFileCollecter([], [:], task)
    ...
    private List<String> fetchResultFiles(Map opts=[:], String namePattern, Path folder) {
        def collector = new TaskFileCollecter([], opts, Mock(TaskRun))
        return collector
    ...
    def defaultOpts(Map opts) {
        return [followLinks: true, glob: true] + opts
    }

    def defaultCollector(Map opts) {
        return new TaskFileCollecter([], defaultOpts(opts), Mock(TaskRun))
    }
    ...
    def 'should collect output files' () {
        ...
        and:
        def opts = defaultOpts(optional: OPTIONAL)
        def collector = Spy(new TaskFileCollecter([FILE_NAME], opts, task))
