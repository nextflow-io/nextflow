...
class CmdLineageTest extends Specification {
    ...
    def 'should warn if no lid content' (){
        ...
        then:
            stdout.size() == 1
            stdout[0] == "Error loading lid://12345. Cid object 12345 not found."
...
"""
        ...
        when:
        def lidCmd = new CmdLineage(launcher: launcher, args: ["lineage", "lid://12345/file.bam", outputHtml.toString()])
        lidCmd.run()
        def stdout = capture
            .toString()
            .readLines()// remove the log part
