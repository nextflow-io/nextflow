...
class OperatorImplTest extends Specification {
    ...
    def testCountBy() {
        expect:
        Channel.of('hello','ciao','hola', 'hi', 'bonjour').countBy { it[0] } .val == [c:1, b:1, h:3]
    }
    ...
    def testGroupBy() {
        ...
        when:
        result = Channel.of('hello','ciao','hola', 'hi', 'bonjour').groupBy { String str -> str[0] }
        then:
        result.val == [c:['ciao'], b:['bonjour'], h:['hello','hola','hi']]

        when:
        result = Channel.of( [id: 1, str:'one'], [id: 2, str:'two'], [id: 2, str:'dos'] ).groupBy()
        then:
        result.val == [ 1: [[id: 1, str:'one']], 2: [[id: 2, str:'two'], [id: 2, str:'dos']] ]

        when:
        result = Channel.of( [1, 'a' ], [2, 'b'], [1, 'c'], [1, 'd'], [3, 'z'] ).groupBy()
        then:
        result.val == [1: [[1,'a'], [1,'c'], [1,'d']], 2: [[2,'b']], 3: [[3,'z']]]
        result instanceof DataflowVariable

        when:
        result = Channel.of( [1, 'a' ], [2, 'b'], [3, 'a'], [4, 'c'], [5, 'a'] ).groupBy(1)
        then:
