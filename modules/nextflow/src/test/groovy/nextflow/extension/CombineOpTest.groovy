...
class CombineOpTest extends Specification {
    ...
    def 'should combine channels' () {
        ...
        when:
        def result = op.apply()
        def all = (List) ToListOp.apply(result).unwrap()
        then:
        all.size() == 9
        ['a', 1] in all
        ['a', 2] in all
    ...
    def 'should combine items'() {
        ...
        def right = ['a','b']
        def result = left.combine(right).toSortedList().unwrap().iterator()
        then:
        result.next() == [1, 'a']
        result.next() == [1, 'b']
        result.next() == [2, 'a']
        ...
        result = left.combine(right).toSortedList().unwrap().iterator()
        then:
        result.next() == [1, 'a']
        result.next() == [1, 'b']
        result.next() == [1, 'c']
    ...
    def 'should chain combine'() {
        ...
        def result = Channel.of(1,2).combine(str1).combine(str2).toSortedList().unwrap().iterator()
        then:
        result.next() == [1,'a','x']
        result.next() == [1,'a','y']
        result.next() == [1,'b','x']
        ...
        str2 = Channel.of('x','y')
        result = Channel.of(1,2).combine(str1).combine(str2,flat:false).toSortedList().unwrap().iterator()
        then:
        result.next() == [1,'a','x']
        result.next() == [1,'a','y']
        result.next() == [1,'b','x']
    ...
    def 'should combine by first item' () {
        ...
        all = (List) ToListOp.apply(result).unwrap()
        then:
        all.size() == 4
        [1, 'a', 'r'] in all
        [1, 'c', 'r'] in all
