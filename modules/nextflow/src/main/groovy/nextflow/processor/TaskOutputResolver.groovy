...
@Slf4j
@CompileStatic
class TaskOutputResolver implements Map<String,?> {
    ...
    String env(String name) {
        final result = env0(task.workDir, null).get(name)
    ...
    static private Map env0(Path workDir, Map<String,String> evalCmds) {
    ...
    Path file(Map opts=[:], String key) {
        ...
        final allFiles = new TaskFileCollecter([filePattern], opts, task).collect()

        if( allFiles.isEmpty() ) {
            if( opts.optional )
                return null
            throw new IllegalArityException("Process output file '${filePattern}' in `${task.lazyName()}` yielded 0 files and was not marked as optional")
        }
     ...
     * @param key
     */
    Set<Path> files(Map opts=[:], String key) {
        final param = declaredFiles.get(key)
        final filePattern = param.getFilePattern(delegate)
        if( filePattern.startsWith('/') )
        ...
        if( allFiles.isEmpty() && !opts.optional )
            throw new IllegalArityException("Process output glob '${filePattern}' in `${task.lazyName()}` yielded 0 files and was not marked as optional")
