...
class TaskProcessor {
    ...
    synchronized protected void bindOutputs( TaskRun task ) {
        ...
        for( OutParam param: task.outputs.keySet() ){
            ...
            case OptionalParam:
                ...
                if( value == null && parentParam instanceof OptionalParam && parentParam.optional ) {
    ...
    protected void collectOutFiles( TaskRun task, FileOutParam param, Path workDir, Map context ) {
        ...
        if( param.optional && allFiles.size() == 0 ) {
            task.setOutput( param, null )
        } else if( allFiles.size() == 1 ) {
            task.setOutput( param, allFiles[0] )
        } else {
            task.setOutput( param, allFiles )
        }
