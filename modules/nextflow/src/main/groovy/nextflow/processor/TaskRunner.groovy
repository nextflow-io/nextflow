...
class TaskRunner {
     ...
     * resolved once, on the first task hashed by this runner. See {@link #createTaskHasher}.
     *
     ...
     * once, on the first task resolved by this runner. See {@link #getCacheStrategy}.
     */
    ...
    /** The {@link TaskResolver} adapter over this runner, created on first use. */
     ...
     * this runner: with no plugin strategy applying, the {@link DefaultTaskCacheStrategy} runs
     ...
     * resolution. Resolved once, on the first task of this runner.
     */
    @CompileStatic
    ...
    /**
     * The {@link TaskResolver} a {@link TaskCacheStrategy} resolves the tasks of this runner with.
     */
    @CompileStatic
    ...
    /**
     * Adapts the resume / launch primitives of this runner to the {@link TaskResolver} contract,
     * so a strategy never touches the runner itself.
     */
    @CompileStatic
    private class Resolver implements TaskResolver {
        ...
        TaskEntry entry(HashCode key) {
            return session.cache.getTaskEntry(key, processor)
        }
