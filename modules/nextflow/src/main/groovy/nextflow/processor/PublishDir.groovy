...
class PublishDir {
    ...
    protected Map getRetryOpts() {
        def result = session.config.navigate('workflow.output.retryPolicy') as Map
        if( result != null )
            log.warn 'The `workflow.output.retryPolicy` config settings have been moved to `nextflow.retryPolicy`'
        else
            result = session.config.navigate('nextflow.retryPolicy') as Map ?: Collections.emptyMap()
        return result
    }
