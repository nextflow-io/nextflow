...
class StateObj implements Serializable, Cloneable {
    ...
    void incSubmitted() {
        if( poisoned )
            log.debug "Error: Cannot process more messages after Poison-Pill was received"
        else
            submitted++
    }
    ...
    void incCompleted() {
        if( completed >= submitted ) {
            log.debug "Error: Processed messages ($submitted) should not overcome received messages ($submitted) count"
        }
        completed++
    }
