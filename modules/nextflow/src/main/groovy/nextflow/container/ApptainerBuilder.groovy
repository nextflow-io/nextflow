...
@CompileStatic
@Slf4j
class ApptainerBuilder extends ContainerBuilder<ApptainerBuilder> {
    ...
    @Override
    ApptainerBuilder params(Map params) {
    ...
    ApptainerBuilder addRunOptions(String str) {
        runOptions.add(str)
        return this
    }

    @Override
    ApptainerBuilder build(StringBuilder result) {
    ...
    protected String prefixEnv(String key) {
        if( key.startsWith('APPTAINER_') )
            return key
        if( key.startsWith('APPTAINERENV_') )
            return key
        return "APPTAINERENV_$key"
    }
