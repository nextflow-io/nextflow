...
class K8sTaskHandler extends TaskHandler {
    ...
    protected boolean legacyEntrypoint() {
        return executor.getK8sConfig().legacyEntrypoint()
    }
    ...
    protected Map newSubmitRequest0(TaskRun task, String imageName) {
        ...
        // when `legacyEntrypoint` is false the launcher is run via `args` instead of `command`
