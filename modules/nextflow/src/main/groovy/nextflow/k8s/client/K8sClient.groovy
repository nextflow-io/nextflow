        ...
        while ( attempt < maxAttempts ) {
            ...
            try {
                ...
                if ( e instanceof K8sResponseException && e.response.code != 500 )
                    throw e
                log.warn1 "[K8s] API request threw socket exception: $e.message for $method $path ${body ? '\n'+prettyPrint(body).indent() : ''}"
                if ( attempt >= maxAttempts )
                    throw e
                log.debug "[K8s] Retrying API request, remaining attempts: ${ maxAttempts - attempt }"
