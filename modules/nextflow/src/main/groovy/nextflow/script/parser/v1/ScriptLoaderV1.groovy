...
/**
 * Legacy script loader based on the Groovy parser.
 *
 * @author Paolo Di Tommaso <paolo.ditommaso@gmail.com>
 */
@CompileStatic
class ScriptLoaderV1 implements ScriptLoader {
    ...
    ScriptLoaderV1(Session session) {
        this.session = session
        this.classLoader = session.classLoader
    }

    ScriptLoaderV1(ClassLoader loader) {
        this.classLoader = loader
    }

    ScriptLoaderV1 setSession( Session session ) {
        this.session = session
        this.classLoader = session.classLoader
        return this
    }
    ...
    @Override
    ScriptLoaderV1 setModule(boolean value) {
        this.module = value
        return this
    }

    @Override
    ScriptLoaderV1 setEntryName(String name) {
        this.entryName = name
        return this
    }

    ScriptLoaderV1 setBinding(ScriptBinding binding) {
    ...
    private ScriptLoaderV1 parse0(String scriptText, Path scriptPath, GroovyShell interpreter) {
    ...
    ScriptLoaderV1 parse(String scriptText) {
        return parse0(scriptText, null, getInterpreter())
    }

    @Override
    ScriptLoaderV1 parse(Path scriptPath) {
        try {
    ...
    ScriptLoaderV1 runScript(String scriptText) {
        parse(scriptText)
        runScript()
        return this
    }

    ScriptLoaderV1 runScript(Path scriptPath) {
        parse(scriptPath)
        runScript()
        return this
    }
