...
public class ScriptCompiler {
    ...
    private static CompilerConfiguration getConfig(boolean debug, Path targetDirectory) {
        ...
        importCustomizer.addImports( "java.nio.file.Path" );
        importCustomizer.addImports( "nextflow.Channel" );
        importCustomizer.addImports( "nextflow.util.Duration" );
        importCustomizer.addImports( "nextflow.util.MemoryUnit" );
    ...
    private CompileResult compile0(GroovyCodeSource codeSource) throws IOException {
        ...
        var classes = (List<Class>) collector.getLoadedClasses().stream()
            .map((o) -> 
                o instanceof Class c && "nextflow.script.BaseScript".equals(c.getSuperclass().getName())
                    ? c
                    : null
            )
            .filter(c -> c != null)
