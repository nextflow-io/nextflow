...
description: Configure Nextflow's Git integration for accessing private repositories.
---

# Git integration

Nextflow integrates with Git to manage pipeline versioning and to pull pipelines directly from source code repositories. This page describes how to configure credentials and other settings for Git providers.
...
The `httpClient` section sets the timeouts for API requests to the Git hosting service. These settings apply to every provider:
...
Maximum time to wait for a connection to the Git hosting service API, for a single attempt (default: `60 sec`). Must be greater than zero. Can also be set with the `NXF_HTTPCLIENT_CONNECT_TIMEOUT` environment variable.

##### `httpClient.requestTimeout`

<AddedInVersion version="26.10" />

Maximum time to wait for a response from the Git hosting service API after the connection is established, for a single attempt (default: `60 sec`). Set to `0 sec` to wait indefinitely. Can also be set with the `NXF_HTTPCLIENT_REQUEST_TIMEOUT` environment variable.

:::note
These settings cannot be set in `nextflow.config`. Nextflow resolves the pipeline repository before it parses the pipeline config. Only the SCM config file and environment variables are available at that point.
:::
