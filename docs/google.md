...
The boot disk type can be specified using the `google.batch.bootDiskType` config option (e.g., `pd-standard`, `pd-balanced`, `pd-ssd`, `hyperdisk-balanced`). See the [Google Batch documentation](https://cloud.google.com/compute/docs/disks) for more information about the available disk types.
