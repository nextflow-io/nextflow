...
- **`'module_run'`** — enables a single generic `module_run(module, args)` tool. The runnable modules are discovered automatically from the script's `include` statements and any locally-defined in-scope processes. The model sees a `module` enum listing all available modules and calls the one it needs.
...
To make a module available to `module_run`, either **include** it with an `include` statement or **define** a process in the same script. Both forms are discovered automatically — no extra annotation is needed.

When the model calls `module_run`, Nextflow marshals the model's JSON arguments into channel values, runs the chosen module as a **real dataflow node** (through the normal executor / container / cache machinery — the task runs in its own work directory), and serializes the module's outputs back to the model as a JSON object so it can continue the conversation and produce a final answer. File outputs are returned as **absolute path-handle strings**, never file contents — so the model passes opaque path handles between tool calls and never sees the bytes of a file.

:::note
`bash` and `nextflow run` are **not** available as agent tools. Agents interact with the pipeline only through the `module_run` and `filesystem` capabilities.
:::

```groovy
nextflow.enable.types = true

// `uppercase` is an in-scope process; `module_run` discovers it automatically.
process uppercase {
    input:  text: String
...
When the model calls `module_run` with `{"module":"uppercase","args":{"text":"hello"}}`, the process executes as a dataflow node and its result flows back to the model, which produces the final `answer` (here, `HELLO`).

#### Using included modules

For modules sourced from the nf-core registry (or any other module file), add an `include` statement so the process is in scope. Multiple included modules all become entries in the `module_run` enum:
...
agent triage {
    ...
    instruction '''
        ...
        Use module_run for each step; use the filesystem tool to write a
        summary report.
    '''
...
The `module` enum exposed to the LLM lists `skesa`, `assemblyscan`, and `prokka`. The model picks which to call and in what order — the adaptive logic is a reasoning step, not a hard-coded DAG edge.

#### The filesystem tool

When `'filesystem'` is declared, the model gains four file operations sandboxed to its per-invocation work directory (plus module-output paths returned by `module_run`):
...
This is useful for reading small structured outputs (JSON, TSV) that `module_run` produces and for writing intermediate or summary files within a single agent invocation.

#### Tool input/output schema

The `module_run` tool advertises a JSON schema for each module's arguments so the model knows how to call it:

- **From a sibling `meta.yml`** — when a module has a sibling `meta.yml`, its input/output spec drives the per-module argument schema, including per-item **descriptions**. Tuple inputs are *flattened* to top-level properties: a `tuple(val(meta), path(reads))` input becomes `{meta, reads}`, where `meta` is an object and `reads` is a path-handle string. The model passes `{"meta": {...}, "reads": "/abs/path"}` rather than a nested array; on the way back, tuple/file outputs are serialized to a JSON object with file values as absolute path strings.
- **From a typed process** — when no `meta.yml` is present, the schema is derived from the typed process's declared scalar I/O: each input parameter `name: Type` becomes a property, and the single scalar output is returned under its name. Tuple/`path`/`meta` process I/O without a `meta.yml` is not supported and fails loudly.
...
| `tools` | no | One or more capability strings enabling tools the agent may call. `'module_run'` enables the generic `module_run(module, args)` tool that runs any in-scope process (discovered from `include` statements and locally-defined processes). `'filesystem'` enables a sandboxed read/write/list/exists tool. Multiple capabilities are allowed: `tools 'module_run', 'filesystem'`. Requires a plain (non-record) output type. See [Tools (calling modules)](#tools-calling-modules). |
...
- `bash` and `nextflow run` are **not** available as agent tools. The only way an agent interacts with pipeline execution is through the `module_run` and `filesystem` capabilities.
