...
description: Read, write, copy, and manage files and directories in Nextflow scripts.
---
...
The `file()` function returns a [Path][stdlib-types-path] object, which has several methods for retrieving metadata about the file:
...
The `withReader()` method creates a [Reader](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/io/Reader.html) that you can use to read the file line by line, or even character by character.
It is useful when you don't need to read the entire file.
...
You can use the `append()` method or left shift (`<<`) operator to append text to a file without erasing the existing contents:
...
See the [Path][stdlib-types-path] reference for the complete list of methods for performing filesystem operations.
