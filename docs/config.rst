...
Advanced Batch configuration options can be set by using the ``azure`` attribute. The following properties can be used (required Nextflow version ``21.4.0`` or later):
...
Name                                           Description
============================================== =================
azure.azcopy.blockSize                          The block size (in MiB) when uploading to Azure Storage, and downloading from Azure Storage (requires ``nf-azure@0.13.1``).
azure.azcopy.blobTier                           The blob tier to be used while uploading to Azure Storage (requires ``nf-azure@0.13.1``).
