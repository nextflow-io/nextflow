...
Wave allows the provisioning of containers based on the `process-spack` directive used by the processes in your
...
:::note
When using a private repository, the repository access keys must be provided via the Seqera Platform credentials manager (see [above ](wave-authenticate-private-repos)).
...
Wave containers allows you to run your containerised workflow with the `fusion-page`.
