...
description: Create modules and share them through the Nextflow module registry.
---
...
The namespace and name must be lowercase, and may contain digits, dots, underscores, and hyphens. The name of the process or workflow *definition* in `main.nf` is independent of the module name -- by convention it is the module name in uppercase, e.g. `HELLO` for module `myorg/hello`.
...
By default this scaffolds a *process* module with static typing. Use `-kind Workflow` to scaffold a *workflow* module, and `-legacy` to scaffold a module without static typing.

```console
$ nextflow module create myorg/my-workflow -kind Workflow
```
...
- The module must define a single process or named workflow
- The module may define an entry workflow to override the default behavior of `module run`.
- The module may define any number of functions and types

Local scripts can define any number of processes, workflows, functions, and types. As a best practice, each process and named workflow should be defined in its own script.
...
When running the module with [Wave][wave-page], the contents of `resources/` are mounted into the root directory of the task container.
...
Modules can include process script [templates][process-template] in the `templates/` directory.
...
For a workflow module, the `input` and `output` sections are derived from the workflow's `take:` and `emit:` declarations. The `requires.modules` section is derived from the module script's include declarations.
...
See [module validate][cli-module-validate] for the full command reference.
...
[cli-module-create]: ../reference/cli/module#module-create
