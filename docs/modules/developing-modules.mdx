...
For a workflow module, the `input` and `output` sections are derived from the workflow's `take:` and `emit:` declarations. Types are inferred only for typed workflows.
