---
title: Workflows
description: Define entry workflows and named workflows in Nextflow.
---
...
:::note
This page describes the original workflow syntax. Nextflow also supports a [typed workflow syntax][workflow-typed-page] with statically typed inputs and outputs. See [Static typing][static-typing-page] for an overview.
:::
...
Labels can be used to find and filter output files across workflow runs with data lineage. See [Data lineage][data-lineage-workflow-outputs] for details on how to query output files by label.
...
When `true`, the keys of the first record are used as the column names (default: `false`). Can also be a list of column names (e.g. `header 'id', 'fastq_1', 'fastq_2'`). Only used for CSV files.
...
Labels are stored in the `labels` field of `FileOutput` records in the [lineage store][data-lineage-page].
...
See the [`channel` namespace][channel-namespace] for the full list of channel factories.
...
- [collect][operator-collect]: collect the channel values into a collection

- [combine][operator-combine]: emit the combinations of two channels

- [filter][operator-filter]: emit only the channel values that satisfy a condition

- [flatMap][operator-flatMap]: emit multiple values for each channel value with a closure

- [grouptuple][operator-grouptuple]: group the channel values based on a grouping key

- [join][operator-join]: join the values from two channels based on a matching key

- [map][operator-map]: transform each channel value with a mapping function

- [mix][operator-mix]: emit the values from multiple channels

- [view][operator-view]: print each channel value to standard output

See [Operators][operator-page] for the full set of operators. See [Value\<V\>][stdlib-types-value] for the set of available methods for dataflow values.
...
Processes and workflows can only be called by workflows. A given process or workflow can only be called once in a given workflow. To use a process or workflow multiple times in the same workflow, [include][syntax-include] it from another script with multiple aliases:
...
These operators are not supported when [static typing][preparing-static-types] is enabled. Use standard method calls and assignments instead.
</DeprecatedInVersion>
...
[channel-namespace]: ./reference/stdlib-namespaces/channel
...
[data-lineage-workflow-outputs]: ./tutorials/data-lineage
...
[preparing-static-types]: ./tutorials/static-types
[process-naming-outputs]: ./process#naming-outputs
[process-out-singleton]: ./process#singleton-outputs
[process-page]: ./process
[process-publishdir]: ./reference/process/directives/publish-dir
[stdlib-types-value]: ./reference/stdlib-types/value
[syntax-include]: ./reference/syntax#include
[workflow-outputs-first-preview]: ./migrations/24-04#workflow-outputs-first-preview
[static-typing-page]: ./static-typing
[workflow-outputs-second-preview]: ./migrations/24-10#workflow-outputs-second-preview
[workflow-outputs-third-preview]: ./migrations/25-04#workflow-outputs-third-preview
