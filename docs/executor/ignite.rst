...
.. warning::
  This feature is no longer maintained due to lack of use. In order to use it in Nextflow 22.01 or later, it must be loaded via the ``nf-ignite`` plugin.
...
.. tip::
    To check if multicast is available in your network, use the `iperf <http://sourceforge.net/projects/iperf/>`_ tool.
