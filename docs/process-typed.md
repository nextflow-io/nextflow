...
:::{note}
This feature requires the {ref}`strict syntax <strict-syntax-page>`. Enable it by setting `NXF_SYNTAX_PARSER=v2`.
:::
...
See {ref}`syntax-process-typed` for the complete syntax reference. To migrate existing code to static types, see {ref}`migrating-static-types`.
