...
By default, Nextflow only selects images that Azure Batch has formally verified. Set `allowUnverifiedImages` to `true` to also allow images that Azure Batch lists but has not verified. This setting is ignored when `virtualMachineImageId` is set.
