...
Also, you need an Azure account and an active subscription to get access to the Azure services used by Nextflow.
...
in the local file system by prepending the file path with the ``az://`` prefix followed by the container name.

For example, given a container named ``my-data`` with a file named ``foo.txt``, you can access the file from your
Nextflow script using the fully qualified path ``az://my-data/foo.txt``.
...
4 - Associate the Storage account with the Batch account.
...
The support for Azure Cloud requires adding the following setting at the beginning of your configuration file::

  plugins {
    id 'nf-azure'
  }

The ``azurebatch`` executor must also be specified in the file::

  process {
    executor = 'azurebatch'
  }

Finally, you need to add the settings to get access to the Batch and Storage accounts.
...
replacing ``<PIPELINE NAME>`` with a pipeline name (e.g. ``nextflow-io/rnaseq-nf`` or ``main.nf``) and ``YOUR-CONTAINER``
with a Blob container available in the storage account defined in the configuration.
...
As an alternative to the account key, the Storage Account can also be accessed via a `Shared Access Token`
by replacing the ``accountKey`` attribute with ``sasToken``.

The token is created in the `Shared Access Signature` setting of the Storage Account. During the creation, allow the
resource types `Container` and `Object` to use the token and set the permissions to `Read`, `Write`, `Delete`, `List`,
`Add`, `Create`.

Differently from the key, the `Shared Access Token` has an expiry date/time. Make sure you create it for the time
needed and/or to renew when it expires.
...
jobs run by your pipeline. By default it uses 1 compute node of ``Standard_D4_v3`` type.
...
The specifics of each pool can be set in your configuration as shown below ::
...
See `Azure Batch <https://docs.microsoft.com/en-us/azure/batch/batch-automatic-scaling>`_ documentation for details.
