...
Nextflow provides built-in support for Cloud Batch, which allows the seamless deployment of Nextflow pipelines
...
* The Google Cloud ``region`` or ``zone``. This is where the Compute Engine VMs will be started.
  You need to specify **one or the other**, not both. Multiple regions or zones can be specified by
