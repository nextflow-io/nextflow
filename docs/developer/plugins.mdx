...
See the [plugin][cli-plugin] CLI command for usage information.
...
While this approach is not required to support plugin config options, it allows Nextflow to recognize plugin definitions when validating the config. See [Configuration scopes][config-scopes] for more information.
...
Plugins can define custom filesystems that can be used by Nextflow to interact with external storage systems using a single interface. For more information about accessing remote files, see [Remote files][remote-files].
...
Nextflow does not provide the ability to define custom process directives in a plugin. Instead, you can use the [ext][process-ext] directive to provide custom process settings to your executor. Try to use specific names that are not likely to conflict with other plugins or existing pipelines.
...
[config-scopes]: /nextflow_docs/nextflow_repo/docs/developer/config-scopes.mdx
