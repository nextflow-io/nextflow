...
In the above example, `hello_bye` takes a channel of files (`Channel<Path>`) and emits a dataflow value with a single file (`Value<Path>`). For more information about the available types, see [Types][stdlib-types].
