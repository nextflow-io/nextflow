...
A `TaskRun` record lists the task's inputs but not its outputs. A task LID *is* the [task hash][cache-resume-task-hash], which is computed from the inputs, script, and environment that determine the task -- output values do not contribute to it. Task results are therefore recorded as a separate lineage entry, retrieved with the `#output` fragment of the task LID. For example, view the outputs of the `QUANT` task that produced one of the inputs above:
...
Each entry is one declared task output: `type` is the output qualifier (`path`, `val`, `env`, `eval`, or `stdout`), `name` is the declared output name (`null` when the output declares only a file pattern), and `value` is a `lid://` reference for file outputs or the literal value otherwise.

:::note
These are the task outputs in the work directory, which are distinct from the workflow outputs published by the run. See [Use lineage with workflow outputs](#use-lineage-with-workflow-outputs).
:::
...
Every run records a `WorkflowRun`, a `TaskRun` (or `AgentRun`) and a `TaskOutput` for each task, and a `FileOutput` for each task output file in the work directory, whether or not the pipeline declares an `output` block. The `output` block adds the `WorkflowOutput` record and guarantees that published files are recorded:
...
When lineage is enabled, Nextflow writes each record to the store as a `.data.json` file in a directory named after the record's lineage ID. Everything after the `lid://` prefix becomes the directory path, so `lid://<hash>` maps to `<store>/<hash>/.data.json` and `lid://<hash>/report.html` maps to `<store>/<hash>/report.html/.data.json`.
...
:::note
`<store>` is the lineage store location, which defaults to `.lineage` in the launch directory. The `#` in `<workflow-hash>#output` and `<task-hash>#output` is a literal character in the directory name.
:::
