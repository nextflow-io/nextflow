---
title: Using operators with static typing
description: Best practices for using operators with static typing in Nextflow.
---
...
The [core operators][channel-type] are recommended for use with static typing. When static typing is enabled (via `nextflow.enable.types`), some of these operators have stricter semantics which may require minor changes to pipeline code. These cases are described below.
...
The [legacy operators][operator-page] are discouraged from use with static typing. They can still be used, but the type checker will not be able to validate them.
...
These operators are [non-deterministic][cache-nondeterministic-inputs]. Use `groupBy` or `List::collate()` instead.

```nextflow
// before
channel.fromList(1..9)
    .collate(3)
    .view()
...
The `collectFile` operator is useful for collecting intermediate results into a final output file, or writing a samplesheet. In many cases, `collectFile` can be replaced by a [workflow output][workflow-output-def], which can generate an index file for a published channel.
...
- Use an `exec` process to write files (see [Working with files][working-with-files])
...
This operator is [non-deterministic][cache-nondeterministic-inputs]. Use `unique` instead.
...
These operators are [non-deterministic][cache-nondeterministic-inputs]. Use the corresponding [List][stdlib-types-list] methods instead.
...
This operator is [non-deterministic][cache-nondeterministic-inputs]. Use `join` instead.
...
This operator is [non-deterministic][cache-nondeterministic-inputs]. It should not be used.
...
Use the equivalent [Path][stdlib-types-path] method with `flatMap` instead:
...
[migrating-static-types]: ./static-types
...
[stdlib-types-iterable]: ../reference/stdlib-types/iterable
[stdlib-types-list]: ../reference/stdlib-types/list
