...
:::{note}
Nextflow 25.10 does not require the use of static types -- they are completely optional, and existing code will still work.
:::
...
Static types are most effective when used with the [Nextflow language server](https://github.com/nextflow-io/language-server), especially with the [VS Code extension](https://marketplace.visualstudio.com/items?itemName=nextflow.nextflow).
...
The VS Code extension provides a command for automatically migrating Nextflow pipelines to static types. To migrate a project, open the Command Palette, search for **Convert pipeline to static types**, and select it.

:::{note}
The extension can also convert individual scripts using the **Convert script to static types** command.
:::

The conversion consists of the following steps:

- Legacy parameter declarations are converted to a `params` block. If a `nextflow_scham.json` file is present, it is also used to infer parameter declarations.
...
Use the schema above to convert the legacy parameters to the equivalent `params` block:
...
To migrate a named workflow to static types, specify the type of each input in the `take:` section. You can infer these types by examining how the workflow is called.
...
Let's examine the arguments that are passed to `RNASEQ` when it is called by the entry workflow:
...
Specify the type for each input as follows:
...
Let's use the same approach again to infer the output types:
...
Specify the type of each output as follows:
