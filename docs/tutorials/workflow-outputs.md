...
The `label` directive assigns a metadata tag to every file published by an output. Labels are recorded in the lineage store and can be used to find and filter published files across workflow runs.

For example, add a label to the `multiqc_report` output:
...
The `label` directive can be specified multiple times to assign more than one label to an output:
...
Labels are stored in the `labels` field of each `FileOutput` record in the lineage store. See {ref}`data-lineage-page` for details on how to query published files by label.
