...
description: Migrate from the publishDir directive to the workflow output definition.
---

# Migrating to workflow outputs

The [workflow output definition][workflow-output-def] is a new way to define the top-level outputs of a workflow. It is a replacement for the [publishDir][process-publishdir] directive. This tutorial demonstrates how to migrate from `publishDir` to workflow outputs using the [rnaseq-nf](https://github.com/nextflow-io/rnaseq-nf) pipeline as an example.

:::note
Workflow outputs are stable in Nextflow 25.10. However, for large pipelines, it is recommended that you migrate to static typing and records first. See [Migrating to static typing][migrating-static-types] for more information.
:::
