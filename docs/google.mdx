---
title: Google Cloud
description: Configure and run Nextflow pipelines on Google Cloud.
---

# Google Cloud

Nextflow integrates with Google Cloud to enable scalable pipeline execution using Google Cloud Batch and data storage using Google Cloud Storage. This page describes how to configure credentials and set up your environment for running Nextflow on Google Cloud.
...
:::warning
Price-based machine type selection is deprecated as of version 26.10 and will be removed in version 27.04. It relies on
the Seqera Cloud Info service, which does not cover every Google Cloud region. From version 27.04, `cpus` and `memory`
will be passed to Google Batch, which selects a machine type itself.

To adopt the new behavior now, set the `NXF_CLOUDINFO_ENABLED` environment variable to `false`. To keep control over the
machine type, specify the `machineType` directive.
:::
...
:::warning
Machine type patterns are deprecated as of version 26.10 and will be removed in version 27.04, because matching a
pattern requires the list of machine types from the Seqera Cloud Info service. Specify a machine type by name instead.
:::
