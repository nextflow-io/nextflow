...
Selector expressions are [Java regular expressions](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html) and support any construct supported by Java, such as globs (`.*`) or *negative lookahead* `(?!...)` to match multiple process names.
