...
In each case, use the fully-qualified process name instead, or a regular expression.

Selector expressions are [Java regular expressions](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html) and must match the entire process name. For example, `.*:PARENT:hello` matches `GRANDPARENT:PARENT:hello`, and a negative lookahead excludes a branch: `PARENT:(?!SKIP:).*:hello` matches `hello` in any subworkflow of `PARENT` except `PARENT:SKIP`.
