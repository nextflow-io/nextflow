---
title: Configuration
description: Configure Nextflow pipelines using configuration files, scopes, profiles, and environment variables.
---
...
Selector expressions are [Java regular expressions](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html) and must match the entire process name or label. This means you can use any construct supported by Java, such as a *negative lookahead* `(?!...)`, to exclude part of a fully qualified name while still matching the rest. The `!` operator negates the whole expression, whereas a lookahead lets you exclude a specific branch of the workflow:

```groovy
process {
    withName: 'MAIN:(?!QC:).*:SAMTOOLS_SORT' {
        memory = 16.GB
    }
}
```

This configuration applies to `SAMTOOLS_SORT` when invoked by any workflow under `MAIN` (e.g. `MAIN:ALIGN:SAMTOOLS_SORT`), *except* when invoked by `MAIN:QC` (e.g. `MAIN:QC:SAMTOOLS_SORT` or `MAIN:QC:DEDUP:SAMTOOLS_SORT`).
...
<AddedInVersion version="25.04">
When using the [strict parser][strict-syntax-page], Nextflow applies profiles in the order you specify them on the command line.
</AddedInVersion>
