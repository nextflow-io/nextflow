...
In each case, use the fully-qualified process name instead. Alternatively, use a regular expression (e.g. `.*:PARENT:hello`) to match multiple variants. You can also use a negative lookahead to exclude a branch, e.g. `MAIN:(?!QC:).*:hello` matches `hello` in any workflow under `MAIN` except `MAIN:QC`.
