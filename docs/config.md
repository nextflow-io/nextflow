...
2. The config file `$HOME/.nextflow/config`, or `$NXF_HOME/.nextflow/config` when `NXF_HOME` is set. See {ref}`NXF prefixed variables <env-vars-nextflow>` for more information.
