...
The `tuple` qualifier groups multiple values into a single input definition. It can be useful when a channel emits [tuples][script-tuples] of values that need to be handled separately. Each element in the tuple is associated with a corresponding element in the `tuple` definition. For example:
...
When a [value channel][channel-type-value] is supplied as a process input alongside a queue channel, the process is executed for each value in the queue channel, and the value channel is re-used for each execution.
...
The `tuple` qualifier outputs multiple values in a single channel as a [tuple][script-tuples]. It is useful when you need to associate outputs with metadata, for example:
