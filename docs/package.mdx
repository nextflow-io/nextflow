...
description: Use the package directive with pluggable package providers (conda, pixi, uv, nix, guix, pak, install2.r) to manage software dependencies.
---
...
Each provider is configured through its own config scope:
...
Provider settings live in the provider's own top-level scope (`conda`, `pixi`, `uv`, `nix`, `guix`, `pak`, `install2r`), not under `packages`.
...
When a process defined in an included module script does not declare a `package` directive, Nextflow automatically looks in the **module directory** for a known manifest file and uses it, analogous to how Wave automatically picks up a `Dockerfile`. This means you can drop a manifest file next to your module and omit the directive entirely:
...
An explicit `package` directive always takes precedence over auto-detection. Auto-detection is skipped for processes defined in the entry script (the project root often holds unrelated manifests) and for processes that already declare a `conda`, `spack` or `container` directive. Other providers expose their manifest file names through the provider plugin API (`getManifestFileNames()`), so additional managers can opt in to auto-detection.
...
|---|---|
| `conda` | `.yml`, `.yaml` (conda env file), any base name; a remote `.yml`/`.yaml` URL also works; or any existing file whose first line is `@EXPLICIT` (conda explicit spec) |
...
- Mamba or Micromamba as an alternative backend (`conda.useMamba`, `conda.useMicromamba`)
...
install2r {
    ...
    repos = 'https://cloud.r-project.org'   // CRAN mirror passed as `install2.r -r`
...
conda env list
conda list --prefix <cacheDir>/env-<hash>
...
- [Conda environments][conda-page] - Legacy conda directive documentation
- [`packages` config scope][config-packages] - Package management configuration options
- [Wave][wave-page] - Wave container integration

[conda-page]: ./conda
[config-packages]: ./reference/config/packages
[wave-page]: ./wave
