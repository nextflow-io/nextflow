---
title: Migrating to the Nextflow plugin registry
description: How to migrate Nextflow plugins from the legacy plugin index to the Nextflow plugin registry.
---
...
3. Replace the contents of `settings.gradle` with the following:
    ...
    Replace `<plugin-name>` with your plugin name.
...
4. In the project root, create a new `build.gradle` file with the following configuration:
    ...
    nextflowPlugin {
        ...
        nextflowVersion = '<minimum-nextflow-version>'
    ...
    - `<plugin-version>`: Your plugin version. For example, `0.5.0`.
    - `<minimum-nextflow-version>`: The minimum Nextflow version required to run your plugin. For example, `25.04.0`.
    - `<provider>`: Your name or organization. For example, `acme`.
    - `<class-name>`: Your plugin class name. For example, `acme.plugin.MyPlugin`.
    - `<extension-point>`: Your extension point identifiers that the plugin will implement or expose. For example, `acme.plugin.MyFactory`.
...
[gradle-plugin-publish]: ./gradle-plugin#publishing-a-plugin
