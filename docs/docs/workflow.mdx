...
See [Operators][operator-page] for the full set of operators.
...
The above snippet defines a process named `greet` and invokes it with the input channel. The result is then piped to the [map][operator-map] operator, which converts each string to uppercase, and finally to the [view][operator-view] operator which prints it.
...
[module-aliases]: ./module#module-aliases
