...
<ChangedInVersion version="23.08.0-edge">
...
<AddedInVersion version="25.11.0-edge" />
...
<AddedInVersion version="26.01.0-edge" />
...
The Seqera Platform compute environment ID. When specified, the scheduler resolves the compute environment directly by this ID instead of inferring a suitable compute environment.
Used as a fallback when the workflow launch does not include a compute environment reference.

###### `seqera.executor.autoLabels`

When `true`, automatically adds workflow metadata labels to the session with the `nextflow.io/` prefix (default: `false`).
...
A `seqera.io/runId` label is also added, computed as a SipHash of the session ID and run name.

###### `seqera.executor.labels`

Custom labels to apply to AWS resources for cost tracking and resource organization.
Labels are propagated to ECS tasks, capacity providers, and EC2 instances. When used together with `autoLabels`, user-defined labels take precedence over auto-generated labels.

###### `seqera.executor.machineRequirement.arch`

The CPU architecture for task execution.
For example, `'x86_64'` or `'arm64'`.

###### `seqera.executor.machineRequirement.provisioning`

The instance provisioning mode.
Can be `'spot'`, `'ondemand'`, or `'spotFirst'`.
...
Only applicable when `diskAllocation` is `'task'`.
...
These are merged with the Fusion environment variables, with Fusion variables taking precedence. For example: `taskEnvironment = [MY_VAR: 'value']`.
...
[trace-fields]: ../reports#trace-fields
