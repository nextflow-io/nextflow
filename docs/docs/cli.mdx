...
description: Use the Nextflow CLI to manage and execute pipelines from local files or remote repositories.
...
2. Configuration parameters (see [Parameters][config-params] for more information)
...
The `module` command enables working with reusable, registry-based modules.
The Nextflow module system allows you to install, run, search, and publish standardized modules from registries, eliminating duplicate work and sharing improvements with the community.
...
Use `-limit` to control the number of results and `-output json` for JSON-formatted output.

See [search][cli-module-search] for more information.

### Installing modules

The `module install` command downloads modules from a registry and makes them available in your workflow.
Modules are stored locally in the `modules/` directory.
An additional `.module-info` file is created during installation to store information such as the module checksum and the registry URL.
...
See [install][cli-module-install] for more information.
...
See [list][cli-module-list] for more information.
...
The output includes the module's version, description, authors, keywords, tools, input/output channels, and a generated usage template showing how to run the module.
Use `-json` for machine-readable output suitable for programmatic access.

See [view][cli-module-view] for more information.

### Running modules directly

The `module run` command executes a module directly from the registry without requiring a wrapper workflow.
This provides immediate access to module functionality for ad-hoc tasks or testing.
...
Process inputs can be specified like params on the command line.
For example, `--reads reads.fq` corresponds to the `reads` input in the `nf-core/salmon` module.
Run `nextflow module view nf-core/salmon` to see the available params for the module.

See [run][cli-module-run] for more information.
...
See [remove][cli-module-remove] for more information.
...
Use the [spec][cli-module-spec] and [validate][cli-module-validate] subcommands to prepare and validate a module before publishing.

See [publish][cli-module-publish] for more information.
...
[cli-module-install]: ./reference/cli#module-install
[cli-module-list]: ./reference/cli#module-list
[cli-module-view]: ./reference/cli#module-view
[cli-module-run]: ./reference/cli#module-run
[cli-module-spec]: ./reference/cli#module-spec
[cli-module-validate]: ./reference/cli#module-validate
[cli-module-remove]: ./reference/cli#module-remove
[cli-module-publish]: ./reference/cli#module-publish
[cli-module-search]: ./reference/cli#module-search
