---
title: Overview
description: Understand Nextflow's core concepts, including processes, dataflow, execution abstraction, and configuration.
---
...
Instead, execution order is determined by their _dependencies_ — `extract_top_hits` depends on the output of `blast_search`, so `blast_search` will be executed first, and then `extract_top_hits`.
...
See [Dataflow][dataflow-page] to learn more.
