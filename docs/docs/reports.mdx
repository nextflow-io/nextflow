---
title: Reports
description: How to generate and interpret Nextflow execution reports, timelines, trace files, and workflow diagrams.
---
...
```bash
nextflow log <run-name> -f [fields]
```
...
Run the `log` command with the `-l` option to see the full list of fields. See [log][cli-log] for more information.
...
```bash
nextflow log <run-name> -t <template> > <report>
```
...
```bash
nextflow log <run-name> -filter '<filters>'
```
...
See [report][config-report] for the list of available config options.
...
The same caveats for the [trace file][trace-report] also apply to the task table.
...
See [timeline][config-timeline] for the list of available config options.
...
See [trace][config-trace] for the list of available config options.
...
See [dag][config-dag] for the list of available config options.

[cli-log]: ./reference/cli#log
[config-dag]: ./reference/config#dag
[config-report]: ./reference/config#report
[config-timeline]: ./reference/config#timeline
[config-trace]: ./reference/config#trace
[metrics-page]: ./tutorials/metrics
[trace-file]: ./reports#trace-file
