...
description: How to configure and run Nextflow pipelines on AWS.
---
...
```bash
nextflow run <script-or-project-name> -bucket-dir s3://<bucket>
```

:::warning
The AWS S3 path needs to contain at least one sub-directory (e.g. `s3://<bucket>/work` rather than `s3://<bucket>`).
:::
