---
title: Migrating to static typing
description: Migrate Nextflow pipelines to static typing using records, type annotations, and typed processes.
---
...
- **Type annotations** can be added to inputs and outputs at every level of a pipeline, from pipeline parameters to process inputs and outputs, using the [standard Nextflow types][stdlib-types]. These annotations make your code easier to understand and are used by the Nextflow language server to identify type-related errors during development.
...
This section demonstrates how to migrate a pipeline to static typing using [rnaseq-nf](https://github.com/nextflow-io/rnaseq-nf) as an example. See [Getting started with rnaseq-nf][rnaseq-nf-page] for an introduction to the pipeline.
...
See [Parameters][workflow-typed-params] for more information about the `params` block.

:::note
Parameters used only in the config file should be declared in the config, not in the script. Since rnaseq-nf has no such parameters, all parameters are declared in the script. See [Parameters][config-params] for more information.
:::
...
:::note
Collection-type params can also be loaded from JSON and YAML samplesheets. See [Parameters][workflow-typed-params] for more information.
:::

### Migrating processes

See [Processes (typed)][process-typed-page] for an overview of typed processes.
...
The tuple output is converted to a record using the `record()` function and specifying a name for each record field. The `path` output qualifier is replaced by the `file()` function (or `files()` if multiple files are expected). See [process outputs][process-reference-typed] for the list of special functions that can be used in the `output:` section to retrieve task outputs.
...
See [Workflows (typed)][workflow-typed-page] for an overview of typed workflows.
...
These channels are emitted as the outputs of `RNASEQ`. However, with records it is usually simpler to join related channels into a single channel (e.g., to publish the channel as a [workflow output][migrating-workflow-outputs]).
...
Finally, the workflow needs to be updated to only emit the `samples_ch` channel. Type annotations are not required for emits, but they are still useful as documentation and as a sanity check -- if the declared output type doesn't match the assigned value's type, the language server will report it.
...
The [strict syntax][strict-syntax-page] is required to use static typing. It is enabled by default in Nextflow 26.04.
...
When preparing for the strict syntax, try to address [deprecation warnings][strict-syntax-deprecated] as much as possible. For example:
...
The [special operators][workflow-special-operators] `|` and `&` provide shorthands for writing dataflow logic:
...
The [each][process-input-each] input qualifier is not supported in typed processes. Use the [combine][operator-combine] operator to create a single tuple channel instead.
...
Many [operators][operator-page] are not statically typed. While you can still use them in typed workflows, the type checker will not be able to fully validate your code. These operators can usually be replaced by another operator and/or a standard library function.

For example, the `splitCsv` operator is not statically typed. Use `flatMap` and the equivalent [Path][stdlib-types-path] method instead:
...
See [Migrating operators][migrating-static-types-operators] for more information.
...
- [Workflows (typed)][workflow-typed-page]
- [Types][stdlib-types]
- [Records][script-records]
- [Record type syntax][syntax-record-type]

[config-params]: ../config#parameters
[devenv-page]: ../developer-env
[migrating-static-types-operators]: ./static-types-operators
[migrating-workflow-outputs]: ./workflow-outputs
[operator-combine]: ../reference/operator#combine
[operator-page]: ../reference/operator
[process-input-each]: ../process#input-repeaters-each
[process-reference-typed]: ../reference/process#inputs-and-outputs-typed
[process-typed-page]: ../process-typed
[rnaseq-nf-page]: ./rnaseq-nf
[script-records]: ../script#records
[strict-syntax-deprecated]: ../strict-syntax#deprecated-syntax
[strict-syntax-page]: ../strict-syntax
[stdlib-types]: ../reference/stdlib-types
[stdlib-types-path]: ../reference/stdlib-types#path
[syntax-record-type]: ../reference/syntax#record-type
[workflow-special-operators]: ../workflow#dataflow
[workflow-typed-page]: ../workflow-typed
[workflow-typed-params]: ../workflow#parameters
