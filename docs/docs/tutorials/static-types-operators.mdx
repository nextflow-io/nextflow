...
These operators are [non-deterministic][cache-nondeterministic-inputs]. Use `List::collate()` instead.
...
- Use an `exec` process to write text files (see [Working with files][working-with-files])
