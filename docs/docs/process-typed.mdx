---
title: Processes (typed)
description: Use the typed process syntax in Nextflow to define statically typed inputs and outputs for processes.
---
...
In this example, the record input is staged as `sample`, and `sample.fastq` is staged as an input file since it is declared with type `Path` in the `Sample` record type.
Each field in the record type is staged into the task the same way as an individual input.
