...
Typed parameters let you declare a pipeline's parameters, their types, and their default values in a single `params` block. Nextflow uses the annotations to validate every parameter reference against its declared type.
...
Dataflow types are useful when a pipeline is [included by another pipeline][pipeline-composition], because they allow the calling pipeline to provide a parameter from dataflow logic. A parameter that is not declared as a `Channel` or `Value` can only receive a regular value.
