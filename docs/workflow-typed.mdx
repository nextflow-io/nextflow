...
`RnaseqOutput` is a record type of the rnaseq outputs, i.e. the type of the record returned by the `NFCORE_RNASEQ()` call. It declares no outputs of its own -- the calling pipeline declares its own `output` block, as it would for any other workflow.
