---
title: Migrating to 25.04
description: Review new features, breaking changes, and deprecations introduced in Nextflow 25.04.
---
...
The *strict syntax parser* is a strict implementation of Nextflow DSL2 based on the [Nextflow language specification][syntax-page]. Originally introduced by the [Nextflow language server](https://github.com/nextflow-io/language-server) alongside Nextflow 24.10, the strict parser is now available in the Nextflow CLI.

The strict parser is disabled by default. Enable it by setting the environment variable `NXF_SYNTAX_PARSER=v2`. See [strict parser][strict-syntax-page] for details.

### Linting and formatting

The `nextflow lint` command checks Nextflow scripts and config files for errors using the [strict parser][strict-syntax-page]. It can also format Nextflow files using the same formatter as the Nextflow language server. See [lint][cli-lint] for details.
...
- New config option: [`aws.batch.terminateUnschedulableJobs`][config-aws-batch-terminateUnschedulableJobs]
- New config option: [`azure.batch.jobMaxWallClockTime`][config-azure-batch-jobMaxWallClockTime]
- New config option: [`fusion.snapshots`][config-fusion-snapshots]
- New config option: [`google.batch.gcsfuseOptions`][config-google-batch-gcsfuseOptions]
- New config option: [`google.batch.networkTags`][config-google-batch-networkTags]
- New config option: [`workflow.output.copyAttributes`][config-workflow-output-copyAttributes]
- New environment variable: [`NXF_PLUGINS_ALLOWED`][env-vars-NXF_PLUGINS_ALLOWED]
- New plugin extension point: [`TraceObserverFactoryV2`][dev-plugins-trace-observers]
- New standard library function: [`env()`][stdlib-env]
- Support [`disk`][process-disk] directive for Azure Batch
...
[config-aws-batch-terminateUnschedulableJobs]: ../reference/config/aws#awsbatchterminateunschedulablejobs
[config-azure-batch-jobMaxWallClockTime]: ../reference/config/azure#azurebatchjobmaxwallclocktime
[config-fusion-snapshots]: ../reference/config/fusion#fusionsnapshots
[config-google-batch-gcsfuseOptions]: ../reference/config/google#googlebatchgcsfuseoptions
[config-google-batch-networkTags]: ../reference/config/google#googlebatchnetworktags
[config-workflow-output-copyAttributes]: ../reference/config/workflow#workflowoutputcopyattributes
[data-lineage-page]: ../tutorials/data-lineage
[dev-plugins-trace-observers]: ../plugins/developing-plugins#trace-observers
[env-vars-NXF_PLUGINS_ALLOWED]: ../reference/env-vars#nxf_plugins_allowed
...
[stdlib-env]: ../reference/stdlib-namespaces/global#env-name-string----string
