...
Maps are hashed differently. The hash now includes both map keys and values, where before it included only the values.

This fixes a bug where changing a map input to a task did not re-execute it. However, any process with a map input will be re-executed on the first resume after upgrading.
...
[config-aws-batch-forceGlacierTransfer]: ../reference/config/aws#awsbatchforceglaciertransfer
[config-executor-onlyJobState]: ../reference/config/executor#executoronlyjobstate
[config-google-batch-installOpsAgent]: ../reference/config/google#googlebatchinstallopsagent
[config-google-batch-logsPath]: ../reference/config/google#googlebatchlogspath
[config-wave-build-conda-baseImage]: ../reference/config/wave#wavebuildcondabaseimage
[config-wave-build-template]: ../reference/config/wave#wavebuildtemplate
