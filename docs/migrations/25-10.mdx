---
title: Migrating to 25.10
description: Review new features, breaking changes, and deprecations introduced in Nextflow 25.10.
---
...
:::note
Typed parameters require the [strict parser][strict-syntax-page]. Set the `NXF_SYNTAX_PARSER` environment variable to `v2` to enable:
...
See [Parameters][workflow-typed-params] for details.

### Workflow outputs (out of preview)

[Workflow outputs][workflow-output-def], introduced in Nextflow 24.04 as a preview feature, have been brought out of preview.
...
:::note
Static typing requires the [strict parser][strict-syntax-page]. Set the `NXF_SYNTAX_PARSER` environment variable to `v2` to enable:
...
In the type system, queue channels are represented as `Channel`, while value channels are represented as `Value`. To make the terminology clearer and more concise, queue channels are now called *dataflow channels* (or simply *channels*), and value channels are now called *dataflow values*.
...
See [Migrating to static typing][migrating-static-types] for details.
...
See [`auth`][cli-auth] and [`launch`][cli-launch] for details.
...
:::note
Plugin developers will not be able to submit PRs to the legacy plugin index once the plugin registry is generally available. Plugins should be updated to publish to the Nextflow plugin registry using the [Nextflow Gradle plugin][gradle-plugin-page] instead of the legacy plugin index. See [plugin registry][plugin-registry-page] for details.
:::

### New syntax for workflow handlers

:::note
The new syntax for workflow handlers requires the [strict parser][strict-syntax-page]. Set the `NXF_SYNTAX_PARSER` environment variable to `v2` to enable:
...
This syntax is simpler and easier to use with the [strict parser][strict-syntax-page]. See [Workflow handlers][workflow-handlers] for details.

### Simpler syntax for dynamic directives

The [strict parser][strict-syntax-page] allows dynamic process directives to be specified without a closure:
...
- The `nextflow.enable.configProcessNamesValidation` feature flag is deprecated. It was originally introduced to suppress false warnings for process selectors targeting conditional processes. The [strict parser][strict-syntax-page] now validates process selectors without producing false warnings.

## Miscellaneous

- New config option: [`docker.registryOverride`][config-docker-registryOverride]
- New config options: [`wave.build.compression.*`][config-wave-build-compression]
- New executor: [Fujitsu Technical Computing Suite (`tcs`)][executor-tcs]
- Support [Java 25][install-requirements]

[aws-java-sdk-v2-page]: ../guides/aws-java-sdk-v2
[cli-auth]: ../reference/cli/auth
[cli-launch]: ../reference/cli/launch
[config-docker-registryOverride]: ../reference/config/docker#dockerregistryoverride
[config-wave-build-compression]: ../reference/config/wave#wavebuildcompressionmode
[config-workflow-handlers]: ../config#workflow-handlers
[dev-plugins-extension-points-config]: ../plugins/developing-plugins#configuration
