...
Nextflow supports `wr <https://github.com/VertebrateResequencing/wr>`_ as an
executor which allows submitting jobs to LSF while taking advantage of LSF
...
The pipeline can be launched either in a local computer or an OpenStack instance.
...
Your options to "deploy" or your wr configuration determine the default image and
allowed flavors that will be used to run tasks. Amongst the allowed flavors, write
will pick the smallest flavor that is large enough to run your tasks, according
to the cpu, memory and disk requirements of each task.
...
details of your S3 system, and wr must also be aware of those details. This can be
arranged most easily by having an ~/.s3cfg file (the file you would need for the
s3cmd tool).
