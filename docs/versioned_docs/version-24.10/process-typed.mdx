...
See [Process (typed)][syntax-process-typed] for the complete syntax reference and [Migrating to static types ][migrating-static-types] to migrate existing code to static types.
...
All [standard types][stdlib-types] except for the dataflow types (`Channel` and `Value`) can be used as type annotations in processes.
...
The `stub:` section behaves the same way as [legacy processes][process-stub].
...
[process-reference-typed]: /docs/reference/syntax.mdx#process-typed
[process-directives]: /docs/process.mdx#directives
[process-script]: /docs/process.mdx#script
[process-stub]: /docs/process.html#stub
[stdlib-types]: /docs/reference/stdlib-types.html#types
