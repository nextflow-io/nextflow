...
The extension can perform static type checking. Enable it using the **Nextflow > Type Checking** extension setting. See [Migrating to static types][migrating-static-types] for more information about migrating to static types.
...
The DAG preview is similar to the [workflow diagram][workflow-diagram] that can be generated at runtime using the `-with-dag` option. However, it is different in several ways:
...
The extension can automatically migrate scripts to static types. See [Migrating to static types][migrating-static-types] for details.
...
The language server can recognize plugin definitions, including configuration options and functions, for plugins that use the [Nextflow Gradle plugin][gradle-plugin-page] and require Nextflow >=25.10. Custom factories and operators are not currently recognized.
...
[syntax-page]: /docs/reference/syntax.mdx
