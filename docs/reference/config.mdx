...
:::note
Leave `ociAutoPull` disabled if you are willing to build a Singularity/Apptainer native image with Wave (see the [wave-singularity][wave-singularity] section).
:::
...
*Used only by the [slurm-executor][slurm-executor], [lsf-executor][lsf-executor], [pbs-executor][pbs-executor] and [pbspro-executor][pbspro-executor] executors.*
...
*Used only by the [slurm-executor][slurm-executor] executor.*
...
*Used only by the [lsf-executor][lsf-executor] executor.*
...
This service account will only be used for tasks submitted by Nextflow, not for Nextflow itself. See [google-credentials][google-credentials] for more information on Google Cloud credentials.
...
Additional pod configuration options such as environment variables, config maps, secrets, etc. Allows the same settings as the [process-pod][process-pod] process directive.
...
The `lineage` scope controls the generation of [cli-lineage][cli-lineage] metadata.
...
See [stdlib-types-versionnumber][stdlib-types-versionnumber] for details.
...
:::note
Leave `ociAutoPull` disabled if willing to build a Singularity native image with Wave (see the [wave-singularity][wave-singularity] section).
:::
...
:::note
Leave `ociMode` disabled if you are willing to build a Singularity native image with Wave (see the [wave-singularity][wave-singularity] section).
:::
