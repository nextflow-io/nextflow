...
```
You can use as key any object property defined in the [Lineage metadata model](https://github.com/nextflow-io/nextflow/tree/master/modules/nf-lineage/src/main/nextflow/lineage/model). Additionally, the `type` key is used to refer to a metadata object class. 
For instance, you can look for outputs generated by a workflow annotated with a certain label using the following command. 

```console
$ nextflow lineage find type=FileOutput workflowRun=lid://c914d714877cc5c882c55a5428b510b1 labels=foo
```
We can also use the `find` command to find all the tasks executed by a workflow using the command below:
