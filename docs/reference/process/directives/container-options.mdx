---
title: containerOptions
description: Reference for the `containerOptions` process directive.
---

# `containerOptions`

The `containerOptions` directive specifies additional container options for the underlying container runtime (for example, Docker or Singularity). For example:

```nextflow
process hello_docker {
    container 'busybox:latest'
    containerOptions '--volume /data/db:/db'

    output:
    path 'output.txt'

    script:
    """
    your_command --data /db > output.txt
    """
}
```

This example provides a custom volume mount for a specific process.

:::warning
This directive is not supported by the [Kubernetes][k8s-executor] executor.
:::

[k8s-executor]: ../../../executor/kubernetes
