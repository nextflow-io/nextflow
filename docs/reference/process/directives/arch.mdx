...
:::note
Multiple architectures are used in full only by executors that support multi-architecture builds (e.g. Seqera Platform). Other executors and the `spack`/Wave build strategies use only the first (primary) architecture in the list, so declare the primary architecture first.
:::
