...
Log a readable trace of each [agent][agent-page]'s execution — its turns, the model reasoning and the tool invocations — at `INFO` level; tool inputs and outputs are logged at `DEBUG`. Equivalent to the `agent.trace` configuration setting.
...
This option is no longer supported. Use the [nf-weblog](https://github.com/nextflow-io/nf-weblog) plugin explicitly instead.
</DeprecatedInVersion>
