...
The output uses tagged lines such as `[PIPELINE]`, `[PROCESS]`, `[WARN]`, `[ERROR]`, and `[SUCCESS]`/`[FAILED]` written to standard output.
...
Controls how Nextflow reacts when a plugin's extracted directory does not match the hash recorded in the `plugins.lock` file: `warn` logs a warning once per plugin and continues, `strict` aborts the run — and also aborts when a plugin is loaded that has no entry in the lockfile at all — and `off` disables the lockfile entirely, so that no hashing, pinning, or reporting takes place (default: `warn`). Verification is dormant when no `plugins.lock` file is present.
