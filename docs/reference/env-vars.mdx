...
Agent mode is also enabled automatically when `AGENT` or `CLAUDECODE` is set to a truthy value, as agents typically do. Setting `NXF_AGENT_MODE` overrides that detection in either direction, so `NXF_AGENT_MODE=false` restores the ANSI log for a run that a human watches in an environment an agent created — a detached `tmux` session, for example (default: `false`).
