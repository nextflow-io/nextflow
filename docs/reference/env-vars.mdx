...
When `false`, disable the use of the Seqera Cloud Info service to select the machine type for Google Batch tasks, and
let Google Batch select it instead (default: `true`). Price-based machine type selection will be removed in version
27.04, along with this variable.
