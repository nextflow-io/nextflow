...
:::note
Despite the shared prefix, this setting is unrelated to the other `NXF_AGENT_*` variables: it controls how Nextflow formats its own console output for an AI *coding* agent, and has no connection to the `agent` pipeline construct.
:::
