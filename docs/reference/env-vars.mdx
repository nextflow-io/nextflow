...
Use an automated development build of Nextflow, for testing purposes only. Set to `master` for the latest build of the master branch, or `pr-<number>` for the latest build of a pull request. Builds are published in the [nextflow-dev-builds](https://github.com/nextflow-io/nextflow-dev-builds) repository. The core plugins compiled from the same source are used in place of the released plugin versions, while third-party plugins are downloaded from the plugin registry as usual.
