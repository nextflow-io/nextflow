...
:::note
By default, the JVM disables the Basic authentication scheme for HTTPS proxy tunnelling (see the `jdk.http.auth.tunneling.disabledSchemes` property in `$JAVA_HOME/conf/net.properties`), which prevents proxy credentials from being sent with the HTTPS `CONNECT` request. When proxy credentials are provided, Nextflow automatically clears this restriction, unless the property has been set explicitly. To set it yourself, use the `NXF_OPTS` environment variable, e.g. `NXF_OPTS='-Djdk.http.auth.tunneling.disabledSchemes='`.
:::
