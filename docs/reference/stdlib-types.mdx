...
See [Channels][channel-page] for an overview of channels. See [Channel Factory][channel-factory] and [Operators][operator-page] for the available functions for creating and manipulating channels.
...
A list is an ordered collection of elements. See [Lists](script-list) for an overview.
...
A map associates or "maps" keys to values. Each key can map to at most one value -- a map cannot contain duplicate keys. See [Maps][script-map] for an overview.
