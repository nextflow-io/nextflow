...
See [dataflow-page][dataflow-page] for an overview of dataflow types. See [operator-page][operator-page] for the available methods for channels.
...
*Implemented by the following types: [stdlib-types-bag][stdlib-types-bag], [stdlib-types-list][stdlib-types-list], [stdlib-types-set][stdlib-types-set]*
...
:::danger
  Converting an unordered collection to a list can lead to non-deterministic behavior. Consider using `toSorted()` instead to ensure a deterministic ordering. See [cache-nondeterministic-inputs][cache-nondeterministic-inputs] for more information.
  :::
...
A list is an ordered collection of values of type `E`. See [script-list][script-list] for an overview of lists.
...
A map associates or "maps" keys of type `K` to values of type `V`. Each key can map to at most one value -- a map cannot contain duplicate keys. See [script-map][script-map] for an overview of maps.
...
See [dataflow-page][dataflow-page] for an overview of dataflow types.
