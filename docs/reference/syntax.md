...
Nextflow uses Java-style comments.
...
A multiline comment starts with `/*` and can be found at any position in the line. Characters following `/*` will be considered part of the comment up to the first `*/` closing the comment.
...
These declarations are composed of statements and expressions.
...
:::{warning}
Statements and top-level declarations can not be mixed at the same level. If your script has top-level declarations, all statements (e.g. an entry workflow) must be contained within them.
:::
...
An include clauses may include multiple names:

```groovy
include { foo ; bar as baz } from './some/module'
```

Include clauses can be separated by newlines or semi-colons:
...
Include clauses can also be specified separately:
...
A *named workflow* consists of a name and a body, and may consist of a *main*, *take*, *emit*, and *publish* section:
...
- The publish section can be specified but are intended to be used in the entry workflow (see below).
...
A process may define *directives*, *inputs*, *outputs*, *script*, *shell*, *exec*, and *stub* sections:
...
- A process must define a script, shell, or exec section (see below). Additional sections are optional.

- Directives do not have an explicit section label, but must be defined first.
...
Enum values can be accessed using names an identifiers. For example, `Day.MONDAY` and `Day.TUESDAY`.
...
Each variable has a *scope*, which specifies the area of the code where the variable is accessible and can be used.
...
Process input variables exist for the entire process body. Variables declared in the process script, shell, exec, and stub sections exist only in their respective section. Variables declared with the `def` keyword are an exception and will also exist in the output section.
...
:::{note}
The appropriate way to raise an error is to use the {ref}`error <stdlib-functions>` function:
```groovy
...
Every expression has a *type*, that may be resolved at compile-time or run-time.
...
Double-quoted strings can be interpolated using the `${}` placeholder and can contain any expression:
...
See {ref}`standard library <stdlib-page>` and {ref}`operator <operator-page>` for more examples.
