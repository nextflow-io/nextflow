...
Consists of a single *output statement*, with a name and type. Combine multiple values into a record. An output may also specify a source expression after an equals sign, which collects a file written by the agent instead of asking the model for the value.
