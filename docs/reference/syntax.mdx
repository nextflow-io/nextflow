...
The `workflow` name refers to the entire pipeline defined by the included script, i.e. its params block, entry workflow, and output block. The `params` and `output` names refer to the params block and output block as record types. Each of them must be given an alias:
