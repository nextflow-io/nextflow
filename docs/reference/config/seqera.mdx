...
##### `seqera.executor.requestTimeout`

The maximum time to wait for a response from the Seqera scheduler service, applied to each attempt (default: `'45 sec'`). A timed-out poll or cancel is retried according to `seqera.executor.retryPolicy`; a timed-out task submission is not re-sent, since the scheduler may already be creating the tasks. Set to `'0 sec'` to wait indefinitely.
