...
The directory where the trace file should be saved. The path is resolved against the workflow output directory (`outputDir`). It must be a relative path within the output directory, and it cannot be used when `trace.file` is an absolute path. By default, it is not used and `trace.file` is resolved against the launch directory.
