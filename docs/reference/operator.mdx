...
description: Reference for legacy Nextflow channel operators.
---
...
See [Operators (typed)][operator-typed] for the new typed operator reference. See [Using operators with static typing][static-types-operators] for the migration guide.
...
:::warning
This operator depends on the ordering of values in the source channel. It can lead to [non-deterministic behavior][cache-nondeterministic-inputs] if used improperly.
:::
...
To be more precise, the operator transforms a sequence of tuples like *(K, V1, V2, ..)* into a sequence of tuples like *(K, list(V), list(W), ..)*.
...
```nextflow file=../snippets/join-tuple.nf
```

```console file=../snippets/join-tuple.out
```
...
```nextflow file=../snippets/join-tuple-remainder.nf
```

```console file=../snippets/join-tuple-remainder.out
```
...
:::tip
As a best practice, use a standard assignment (`=`) instead of `set`. Standard assignments enable more effective [type checking][preparing-static-types].
:::
...
:::tip
As a best practice, use a standard assignment (`=`) instead of `tap`. Standard assignments enable more effective [type checking][preparing-static-types].
:::
...
Associate the channel with a tag. For example:
...
[script-regexp]: ../script#regular-expressions
