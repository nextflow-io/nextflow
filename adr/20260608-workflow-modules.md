...
- Authors: Ben Sherman, Jorge Ejarque
- Status: accepted
- Date: 2026-06-08
...
Workflow modules can be published to and queried from the Nextflow registry using the same modules API and `nextflow module` CLI. Workflow modules have the same directory structure as process modules.
...
- `create`: scaffolds a process or workflow module using the `-kind` option (`Process` by default, or `Workflow`).
...
- Related: [Direct execution for named workflows](20260601-named-workflow-execution.md)
