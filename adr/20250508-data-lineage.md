...
Nextflow already produces this provenance information at runtime and saves some of it to the metadata cache and the work directory, but most of it is discarded at the end of a run. Even the cache and work directory are ephemeral by design, not suitable as a persistent store of lineage data.
...
The lineage store is an extension point, so third-party plugins can integrate their own backends into the core lineage system. For example, an alternative backend could store lineage records in a SQLite or Postgres database.
...
Workflow output labels have limited granularity. They can be set per-run or per-output, but not per-sample or per-file. Labels are an experimental feature, and it is not yet clear which use cases coarse-grained labels will satisfy (experiment ID, QC vs alignment results, etc).

A workflow output can also produce an *index file*, which serializes the published channel as a structured file (e.g. CSV, JSON, YAML) containing published file paths and associated metadata. This metadata is a fine-grained alternative to workflow output labels.

### Integration with Seqera Platform

How external systems such as Seqera Platform will present data lineage to users remains to be seen, but the lineage system was designed with the following principles in mind:
...
One option for Nextflow was to adopt the OpenLineage standard for defining lineage records. This would allow Nextflow data lineage to be consumed by existing tools that support OpenLineage, such as [Marquez](https://marquezproject.ai/).

However, it made more sense to develop a Nextflow-specific lineage model that we can adapt to our needs as they change. OpenLineage could be supported in the future as an output format, i.e. adding the ability to export Nextflow lineage records to OpenLineage.
...
This approach presents a dilemma. The output must be recorded separately, because an upstream record cannot depend on a downstream record (the lineage graph must be *acyclic*). But that also means a workflow output cannot be retrieved directly, since it is downstream of the workflow run.
...
The `#output` suffix is part of a wider effort to align the lineage system with the existing runtime:
...
A "pure LID" approach that is closer to the CID model remains an option for future improvement, if it proves worthwhile. The current approach is practical, and it lets us add lineage as a supplemental layer without modifying the core runtime.
...
However, the current approach is useful for experimenting with lineage without touching the existing cache and work directory, which are core components of the Nextflow runtime. Unification is worth investigating later, when the lineage system is more mature.
