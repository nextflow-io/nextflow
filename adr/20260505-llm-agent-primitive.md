...
5. ~~**Tool bridge is unimplemented**~~ — **DELIVERED (Plan G, complete through Phase 4, 2026-06-02).** The full module-as-tool bridge is in place. Core `ModuleToolBridge`/`ToolDescriptor`/`ToolDispatcher` pre-wire each declared tool, marshal the LLM's JSON args into channel values, run the module as a real dataflow node, and serialize the output back to JSON; the plugin's adapter builds langchain4j `ToolSpecification`s and runs the multi-tool tool-call loop. Tools require a non-record output (tools XOR structured output). **Delivered across the bridge:** in-scope (local) process tools, local-file module tools, and registry-reference (`nf-core/...`) tools; `meta.yml`/`ModuleSpec`-driven schemas (tuple flattening, `path` file handles, the `meta` map); multiple tools per agent; dispatch-level error-as-tool-result (a tool failure surfaced through the bridge is returned to the LLM as a tool result, allowing recovery within the loop). See the real-run evidence under "Staged scope" below.
6. ~~**`agent` config scope**~~ — **DELIVERED (Plan G Phase 4, 2026-06-02).** `nextflow.config` `agent { ... }` block is now parsed and applied to agent execution (model/iteration/timeout defaults).
...
1. **Headline slice** — **DELIVERED (Plan G Phase 2, 2026-06-02).** An agent invokes a **local/in-scope process** as a tool, end-to-end with a real LLM, via pre-wire + blocking-pull + poison. The core `ModuleToolBridge`/`ToolDispatcher` derives each tool's input schema from the process's typed inputs and serializes its output back to JSON; the plugin's `LangChainAgentRunner` runs the manual tool-call loop. **Tools XOR structured output:** when `tools` are declared the LLM's free-text answer is emitted, so a record (structured) output is rejected at agent run time (`AgentDef` guard + test). **Verified with a real run** (`NXF_PLUGINS_MODE=dev ./launch.sh run` of a `uppercase`-tool agent against live OpenAI `gpt-5-mini`): the tool loop fired the dispatcher (`LangChainAgentRunner - Agent tool call name=uppercase; args={"text":"hello"}`), the `uppercase` process then ran as a real dataflow node on the executor (`Submitted process > uppercase (1)` with a real work dir, `Task completed`), and the result flowed back to the model to produce the final answer `ANSWER=HELLO`. The whole module-as-tool bridge is now **DELIVERED through Plan G Phase 4** — see the re-verification under "Plan G complete" below.
2. **Agent I/O relaxation** — **DELIVERED.** `val`/`path` I/O allowed; the record structured-output path is opt-in (used only when a record output is declared).
3. **Registry modules** — **DELIVERED (Plan G Phase 3, 2026-06-02).** A tool reference resolves to a `ProcessDef` + `ModuleSpec` for in-scope processes, local-file modules, and registry references (`nf-core/...`); the tool schema is derived from the `meta.yml`/`ModuleSpec` (descriptions, tuple flattening, `path` file handles, the `meta` map). *Validation-environment caveat:* the real registry-download + container-execution E2E (pulling `nf-core/fastqc` and running its container) is exercised only in the validation environment, not in unit tests.
4. **Polish** — **DELIVERED (Plan G Phase 4, 2026-06-02).** Multi-tool advertisement/dispatch, dispatch-level error-as-tool-result, termination edge cases, the `agent {}` config scope, the `docs/agent.md` tool documentation, and a gated (`@Requires(OPENAI_API_KEY)`) real-LLM tool-call-loop E2E test (`AgentToolEndToEndTest`, skipped keyless / passing against live OpenAI) all landed. *Deferred:* parallel/correlated tool calls (v1 serializes calls — outputs reorder under `maxForks>1`); recovery from a tool-**PROCESS** failure (a process that genuinely fails aborts the run rather than feeding the error back to the LLM); non-OpenAI providers (`langchain4j-anthropic`, `-google-ai-gemini`).
...
### Plan G complete — module-as-tool bridge DELIVERED (2026-06-02)

**Status: the headline goal of the feature — *tool calls are real dataflow nodes; tools are modules* — is delivered.** All four staged-scope slices are done (Phase 2 in-scope/scalar, I/O relaxation, Phase 3 registry/`meta.yml` schemas, Phase 4 multi-tool + error-as-tool-result + config scope + docs + gated E2E). The plugin↔core split held: core never imports `dev.langchain4j.*`, the plugin never touches a `ProcessDef`/`Channel`/`Path`.

**Re-verified with a real run (Plan G Phase 4 Task 4.3 Part D).** `NXF_PLUGINS_MODE=dev ./launch.sh -trace nextflow.agent run tool-demo.nf -c agent.config` of an `uppercase`-tool agent (`agent shouty { tools 'uppercase'; ... }`, dev-mode `plugins { id 'nf-agent@0.1.0' }`) against live OpenAI `gpt-5-mini`. Verbatim `.nextflow.log` evidence chain:

```
DEBUG nextflow.agent.LangChainAgentRunner - Running agent model=openai/gpt-5-mini; tools=[uppercase]; maxIterations=20
DEBUG nextflow.agent.LangChainAgentRunner - Agent tool call name=uppercase; args={"text":"hello"}
INFO  nextflow.Session - [6a/bb0452] Submitted process > uppercase (1)
DEBUG n.processor.TaskPollingMonitor - Task completed > TaskHandler[id: 1; name: uppercase (1); status: COMPLETED; ...; workDir: .../work/6a/bb0452c812669e23dfd7f8b16b6a54]
```

Console output: `ANSWER=HELLO`, `[SUCCESS] completed=1 failed=0 cached=0`. The LLM requested the tool, the real `uppercase` process ran on the executor in a genuine work dir, and the result flowed back to the model to produce the final answer.

**Gated E2E coverage:** `AgentToolEndToEndTest` (`@Requires(OPENAI_API_KEY)`) exercises the full tool loop through the real OpenAI API (skipped keyless, passing with the key).

**Deferred (not in v1):** parallel/correlated tool calls (calls are serialized; outputs reorder under `maxForks>1`); recovery from a tool-**PROCESS** failure (a process that genuinely fails aborts the run — the dispatch-level error-as-tool-result handles bridge/dispatch errors, not in-process task failures); non-OpenAI providers; and the real registry-download + container-execution E2E (pulling and running `nf-core/...` containers), which is exercised only in the validation environment.
