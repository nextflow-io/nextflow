...
1. **Agent body resolution** — directives like `model 'gpt-4'`, `instruction '...'`, `tools(fastqc)` and the input/output parameter variables (`q`, `r` referenced in the `prompt:` template) currently fail name resolution under typed mode (`Phases.NAME_RESOLUTION` errors). Needs an `AgentDsl` class (parallel to `ProcessDsl`) declaring the directive method shapes, plus parameter-scope handling in `VariableScopeVisitor.visitAgent`. The end-to-end smoke test ships with a minimal fixture (`agent foo { prompt: "..." }`) that sidesteps this; the full directive form does not yet load cleanly.
...
1. **Plan B: Agent body resolution** — define the `AgentDsl` class with directive methods (`model`, `instruction`, `tools`, `maxIterations`), wire `VariableScopeVisitor.visitAgent` to push the DSL scope and the input parameters as locals before visiting outputs/prompt. This unlocks the full DSL example in the spec, including the directive-rich `AgentScriptLoadingTest` fixture.
...
5. **Polish backlog** (any time): make `agentPrompt` grammar-optional for better errors; add `agent` config scope; add an `@author` tag style sweep across new files; revisit `simpleName` invariants when scoped sub-workflows invoke agents.
